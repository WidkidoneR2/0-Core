...
- `commands/mod.rs:1424` [/run/current-system] -- real process spawn -- a capability that must degrade
  - `nix_query_lines(&["nix-store", "-q", "--references", "/run/current-system/sw"]);`
- `commands/mod.rs:1480` [nixos-rebuild] -- real process spawn -- a capability that must degrade
  - `let out_raw = std::process::Command::new("nixos-rebuild")`
- `commands/mod.rs:7160` [systemctl] -- real process spawn -- a capability that must degrade
  - `let output = std::process::Command::new("systemctl")`
- `commands/mod.rs:7437` [nix-store] -- real process spawn -- a capability that must degrade
  - `let roots = match nix_query_lines(&["nix-store", "--query", "--roots", &path]) {`
- `commands/mod.rs:7460` [nix-store] -- real process spawn -- a capability that must degrade
  - `let refs = nix_query_lines(&["nix-store", "--query", "--referrers", &path])`
- `commands/mod.rs:7537` [nix-store] -- real process spawn -- a capability that must degrade
  - `let dead_out = std::process::Command::new("nix-store")`
- `commands/mod.rs:7627` [nix-store] -- real process spawn -- a capability that must degrade
  - `match nix_query_lines(&["nix-store", "--query", "--roots", p]) {`
- `commands/mod.rs:7684` [/nix/store] -- reads the Nix store directly -- a capability that must degrade
  - `if target.starts_with("/nix/store/") && std::path::Path::new(target).exists() {`
- `commands/mod.rs:7689` [/nix/store] -- reads the Nix store directly -- a capability that must degrade
  - `std::fs::read_dir("/nix/store").map_err(|e| format!("  cannot read /nix/store: {}", e))?;`
- `commands/mod.rs:7816` [journalctl] -- real process spawn -- a capability that must degrade
  - `let mut cmd = std::process::Command::new("journalctl");`
- `commands/mod.rs:7865` [journalctl] -- real process spawn -- a capability that must degrade
  - `let output = std::process::Command::new("journalctl")`
- `main.rs:3135` [nixos-rebuild] -- real process spawn -- a capability that must degrade
...
- `platform.rs:118` [/nix/store] -- inside a #[test] function -- a literal path is correct in a fixture
...
- `triage.rs:236` [/home/christian] -- inside a #[test] function -- a literal path is correct in a fixture
  - `"warning: Git tree '/home/christian/0-core' is dirty",`
- `triage.rs:273` [/nix/store] -- inside a #[test] function -- a literal path is correct in a fixture
  - `cold_fix("error: getting status of '/nix/store/x-source/foo.rs': No such file")`
- `triage.rs:299` [/nix/store] -- inside a #[test] function -- a literal path is correct in a fixture
  - `classify("error: getting status of '/nix/store/x/foo.rs': No such file");`

## C (16)

- `commands/mod.rs:1437` [/run/current-system] -- message or predicate text, not a call
  - `"packages: /run/current-system/sw references nothing"`
- `commands/mod.rs:1534` [nix-env] -- message or predicate text, not a call
  - `"\n    sudo nix-env --switch-generation {} -p /nix/var/nix/profiles/system", n`
- `commands/mod.rs:7484` [/run/current-system] -- message or predicate text, not a call
  - `"/run/current-system"])`
- `commands/mod.rs:7543` [/nix/store] -- message or predicate text, not a call
  - `.filter(|l| l.starts_with("/nix/store"))`
- `commands/mod.rs:7548` [nix-store] -- message or predicate text, not a call
  - `format!("  store reclaim: nix-store failed: {}", e).into(),`
- `commands/mod.rs:9971` [systemctl] -- message or predicate text, not a call
  - `"systemctl",`
- `commands/mod.rs:9972` [pacman] -- message or predicate text, not a call
  - `"pacman",`
- `commands/mod.rs:10114` [systemctl] -- message or predicate text, not a call
  - `"systemctl",`
- `commands/mod.rs:10115` [pacman] -- message or predicate text, not a call
  - `"pacman",`
- `commands/mod.rs:11340` [pacman] -- message or predicate text, not a call
  - `"paru" | "pacman" => Some("💡 That isn't a NixOS command — apply changes with deploy (it re`
- `commands/mod.rs:12713` [/run/current-system] -- message or predicate text, not a call
