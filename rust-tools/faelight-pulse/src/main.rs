...
use ratatui::{
    ...
    text::{Line, Span},
    widgets::{
        Block, Borders, Gauge, List, ListItem, ListState, Paragraph, Sparkline,
    },
    Terminal,
};
...
// ─── CLI ────────────────────────────────────────────────────────────────────
#[derive(Parser)]
#[command(name = "faelight-pulse", about = "🌲 core pulse — the forest watching itself breathe", version = "1.0.0")]
struct Cli {
        ...
        if let Ok(v) = serde_json::from_str::<serde_json::Value>(&self.detail) {
            match self.domain.as_str() {
                "doctor" => {
                    ...
                    return format!("health {}%  {}/22 passed  {} warnings", health, passed, warn);
                }
                "security" => {
...
fn load_events(conn: &Connection, since_id: i64, domain_filter: Option<&str>, limit: usize) -> Vec<PulseEvent> {
...
fn load_health_sparkline(conn: &Connection) -> Vec<u64> {
    let mut stmt = match conn.prepare(
        "SELECT payload FROM events WHERE domain='doctor' ORDER BY id DESC LIMIT 20"
    ) {
        Ok(s) => s,
        Err(_) => return vec![],
    };
...
// ─── APP STATE ───────────────────────────────────────────────────────────────
const DOMAINS: &[&str] = &["all", "doctor", "git", "security", "update", "intent", "checkpoint"];
...
impl App {
    ...
    fn active_filter(&self) -> Option<&str> {
        ...
        if d == "all" { None } else { Some(d) }
    }

    fn poll(&mut self, conn: &Connection) {
        if self.paused { return; }
        if self.last_poll.elapsed() < Duration::from_millis(500) { return; }
...
fn draw(terminal: &mut Terminal<CrosstermBackend<io::Stdout>>, app: &mut App) -> io::Result<()> {
    terminal.draw(|f| {
        ...
        let left = Paragraph::new(Line::from(vec![
            ...
            Span::styled("Core Pulse", Style::default().fg(FG).add_modifier(Modifier::BOLD)),
        ]))
        ...
        let right = Paragraph::new(Line::from(vec![
            Span::styled(format!("{}  ⏱ {}  ", now, app.uptime()), Style::default().fg(DIM)),
        ]))
        ...
        let gauge_color = if health >= 95 { ACCENT } else if health >= 80 { YELLOW } else { RED };
        ...
        let spark = Sparkline::default()
            .block(Block::default()
                .title(Span::styled(" Health Trend (last 20 checks) ", Style::default().fg(DIM)))
                .borders(Borders::ALL)
                .border_style(Style::default().fg(DIM)))
            .data(&spark_data)
        ...
        // ── FILTER BAR ──────────────────────────────────────────────────
        let filter_spans: Vec<Span> = DOMAINS.iter().enumerate().flat_map(|(i, d)| {
        ...
        let paused_span = if app.paused {
            Span::styled("  ⏸ PAUSED  ", Style::default().fg(YELLOW).add_modifier(Modifier::BOLD))
        } else {
        ...
        let filter_bar = Paragraph::new(Line::from(all_spans))
            .block(Block::default()
                .borders(Borders::ALL)
                .border_style(Style::default().fg(DIM))
                .title(Span::styled(" Tab: cycle domain  Space: pause  q: quit ", Style::default().fg(DIM))));
        ...
        let items: Vec<ListItem> = app.events.iter().map(|e| {
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::styled(format!("{:<10}", e.domain), Style::default().fg(color).add_modifier(Modifier::BOLD)),
        ...
        let list = List::new(items)
            .block(Block::default()
                ...
                .title(Line::from(vec![
                    Span::styled(" 🌲 Event Stream ", Style::default().fg(ACCENT).add_modifier(Modifier::BOLD)),
                    Span::styled(format!("({} events) ", event_count), Style::default().fg(DIM)),
                ])))
            ...
            if let Event::Key(key) = event::read()? {
                ...
                match key.code {
                    ...
                    KeyCode::Down => {
                        ...
                        app.list_state.select(Some(i.saturating_add(1).min(app.events.len().saturating_sub(1))));
                    }
                    KeyCode::Up => {
                        let i = app.list_state.selected().unwrap_or(0);
