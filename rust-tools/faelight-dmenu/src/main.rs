...
use ui::{renderer::DmenuApp, DmenuState};
...
fn run_apps_mode(_prompt: &str) {
    ...
    let items: Vec<String> = apps.iter().map(|app| app.name.clone()).collect();
...
fn run_commands_mode(_prompt: &str) {
    ...
    let state = Arc::new(Mutex::new(DmenuState::new(
...
fn run_intents_mode(
    ...
    // Create items list
    let items: Vec<String> = intents.iter().map(|i| i.display_text()).collect();
