...
impl Color {
    ...
    pub const DEFAULT_FG: Self = Self::rgb(0xec, 0xf2, 0xee);
...
impl Terminal {
    ...
    fn put_char(&mut self, ch: char) {
        let cp = ch as u32;
        if matches!(cp, 0xFE00..=0xFE0F | 0x200B | 0x200C | 0x200D | 0x20D0..=0x20FF) {
            return;
        }
...
impl<'a> Perform for VteHandler<'a> {
    ...
    fn csi_dispatch(&mut self, params: &Params, _: &[u8], _: bool, action: char) {
        ...
        match action {
            ...
            'P' => {
                ...
                for i in cx..cols {
                    self.term.grid[row][i] = if i + n < cols {
                        self.term.grid[row][i + n]
                    } else {
                        Cell::default()
                    };
                }
            ...
            'T' => {
                ...
                for _ in 0..n {
                    ...
                    self.term
                        .grid
                        .insert(0, vec![Cell::default(); self.term.cols]);
                }
            }
            'm' => {
                if ps.is_empty() || (ps.len() == 1 && ps[0] == 0) {
                    self.term.cur_fg = Color::DEFAULT_FG;
                    self.term.cur_bg = Color::DEFAULT_BG;
                    self.term.cur_attrs = CellAttrs::default();
                    return;
                }
                let mut i = 0;
                while i < ps.len() {
                    match ps[i] {
                        ...
                        1 => {
                            self.term.cur_attrs.bold = true;
                        }
                        2 => {
                            self.term.cur_attrs.dim = true;
                        }
                        3 => {
                            self.term.cur_attrs.italic = true;
                        }
                        4 => {
                            self.term.cur_attrs.underline = true;
                        }
                        5 => {
                            self.term.cur_attrs.blink = true;
                        }
                        7 => {
                            self.term.cur_attrs.reverse = true;
                        }
                        9 => {
                            self.term.cur_attrs.strikethrough = true;
                        }
                        22 => {
                            self.term.cur_attrs.bold = false;
                            self.term.cur_attrs.dim = false;
                        }
                        23 => {
                            self.term.cur_attrs.italic = false;
                        }
                        24 => {
                            self.term.cur_attrs.underline = false;
                        }
                        27 => {
                            self.term.cur_attrs.reverse = false;
                        }
                        29 => {
                            self.term.cur_attrs.strikethrough = false;
                        }
                        30..=37 => {
                            self.term.cur_fg = color256((ps[i] - 30) as u8);
                        }
                        38 => {
                            if ps.get(i + 1).copied() == Some(5) {
                                if let Some(&idx) = ps.get(i + 2) {
                                    self.term.cur_fg = color256(idx as u8);
                                }
                                i += 2;
                                ...
                                self.term.cur_fg = Color::rgb(r, g, b);
                                i += 4;
                            }
                        }
                        39 => {
                        ...
                        40..=47 => {
                            self.term.cur_bg = color256((ps[i] - 40) as u8);
                        }
                        48 => {
                            if ps.get(i + 1).copied() == Some(5) {
                                if let Some(&idx) = ps.get(i + 2) {
                                    self.term.cur_bg = color256(idx as u8);
                                }
                                i += 2;
                            } else if ps.get(i + 1).copied() == Some(2) {
                                let r = ps.get(i + 2).copied().unwrap_or(0) as u8;
                                let g = ps.get(i + 3).copied().unwrap_or(0) as u8;
                                let b = ps.get(i + 4).copied().unwrap_or(0) as u8;
                                self.term.cur_bg = Color::rgb(r, g, b);
                                i += 4;
                            }
                        }
                        49 => {
                        ...
                        90..=97 => {
                            self.term.cur_fg = color256((ps[i] - 90 + 8) as u8);
                        }
                        100..=107 => {
                            self.term.cur_bg = color256((ps[i] - 100 + 8) as u8);
                        }
                        _ => {}
                    }
                    i += 1;
    ...
    fn esc_dispatch(&mut self, _: &[u8], _: bool, byte: u8) {
        if byte == b'M' && self.term.cursor_y > 0 {
