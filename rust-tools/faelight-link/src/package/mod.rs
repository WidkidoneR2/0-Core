...
pub fn get_stow_dir() -> Result<PathBuf> {
    ...
    Ok(home.join("0-core/03-interfaces/stow"))
}
...
pub fn stow(package: &str, force: bool, dry_run: bool) -> Result<()> {
    ...
    println!(
        "  Package: {}",
        pkg_path.display().to_string().bright_black()
    );
    ...
    if files.len() > 5 {
        println!(
            "    {} {} more...",
            "→".bright_black(),
            (files.len() - 5).to_string().bright_black()
        );
    }
...
pub fn unstow(package: &str, dry_run: bool) -> Result<()> {
    ...
    if links.is_empty() {
        println!(
            "  {} No symlinks found for this package",
            "⚠️".bright_yellow()
        );
        return Ok(());
    }
    ...
    for link in &links {
        println!(
            "    {} {}",
            "→".bright_black(),
            link.display().to_string().bright_black()
        );
    }
