// Input handling for faelight-compositor
// Handles keyboard, pointer motion, pointer buttons, and scroll.
// Pattern taken directly from smallvil — understood, not copied blindly.
...
impl FaelightCompositor {
        ...
        match event {
            ...
            InputEvent::PointerAxis { event, .. } => {
                ...
                if horizontal_amount != 0.0 {
                    ...
                    if let Some(d) = horizontal_discrete {
                        frame = frame.v120(Axis::Horizontal, d as i32);
                    }
                }
                if vertical_amount != 0.0 {
                    frame = frame.value(Axis::Vertical, vertical_amount);
                    if let Some(d) = vertical_discrete {
    ...
    /// Find the surface and its location under a given point.
