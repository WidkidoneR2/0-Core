    ...
    for line in raw.lines() {
        ...
        if s.contains("Forecast") {
            forecast_line = s
                ...
                .trim_start_matches("📈")
                .trim()
                .trim_start_matches("Forecast")
                .trim()
                .to_string();
            continue;
        }
        // Section header: ╭─ 🖥  System  or similar
        let is_section = (s.contains("System")
            || s.contains("Git & Code")
            || s.contains("Git")
            || s.contains("Tools")
            || s.contains("Forest")
...
fn parse_section_header(s: &str) -> (String, String) {
    if s.contains("System") {
        return ("🖥 ".to_string(), "System".to_string());
    }
    if s.contains("Git") {
        return ("🌿".to_string(), "Git & Code".to_string());
    }
    if s.contains("Tools") {
...
fn draw_health_ui(
    ...
    let worst = sections
        .iter()
        .map(|s| s.worst())
        .fold(&Severity::Healthy, |acc, s| match (acc, s) {
...
fn draw_info_panel(
    ...
    let warnings: Vec<(String, String)> = sections
        .iter()
        .flat_map(|s| {
            s.checks
                .iter()
                .filter(|c| c.severity == Severity::Warning || c.severity == Severity::Critical)
    ...
    if !warnings.is_empty() {
        ...
        for (section, detail) in &warnings {
            ...
            let detail_short = if detail.len() > 45 {
                &detail[..45]
            } else {
                detail
            };
    ...
    let items: Vec<ListItem> = sections
        ...
        .map(|sec| {
            let pass = sec
                .checks
                .iter()
                .filter(|c| c.severity == Severity::Healthy)
                .count();
            ...
            let line = Line::from(vec![
                ...
                Span::styled(
                    "  ▸".to_string(),
                    Style::default().fg(Color::Rgb(80, 100, 85)),
                ),
            ]);
            ListItem::new(line)
        })
...
fn draw_expanded(
    ...
    let split = Layout::default()
        .direction(Direction::Vertical)
        .constraints([
            Constraint::Length(sections.len() as u16 + 1),
            Constraint::Min(4),
        ])
        .split(area);
    draw_compact(f, split[0], sections, list_state);
    // Bottom: expanded detail
    let checks: Vec<ListItem> = sec
        .checks
        .iter()
        .map(|c| {
        ...
        .collect();
    let detail = List::new(checks).block(
        Block::default()
            .title(Line::from(vec![Span::styled(
                format!(" {} {} ", sec.icon, sec.name),
                Style::default()
                    .fg(Color::Rgb(107, 227, 163))
                    .add_modifier(Modifier::BOLD),
            )]))
