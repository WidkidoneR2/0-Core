...
pub fn translate(input: &str) -> Option<Translation> {
    ...
    for pattern in PATTERNS {
        for phrase in pattern.phrases {
            ...
            if score > 0.4 && best.as_ref().map(|(s, _, _)| score > *s).unwrap_or(true) {
...
pub fn render_pattern_list() -> String {
    ...
    let categories = [
        (
            "Filesystem",
            &["biggest files", "recent files", "list files"][..],
        ),
        ("Processes", &["memory hogs", "cpu hogs", "all processes"]),
        (
            "Services",
            &["failing services", "running services", "all services"],
        ),
        ...
        (
            "Events",
            &["what happened today", "git events", "shell events"],
        ),
    ...
    out.push_str(&format!(
        "  {}\n",
        "Tip: partial phrases work too — ?slow, ?memory, ?commits"
            .dimmed()
            .italic()
    ));
    out.push('\n');
...
pub fn translate_with_custom(input: &str, custom: &[CustomPattern]) -> Option<Translation> {
    ...
    for pattern in PATTERNS {
        for phrase in pattern.phrases {
            let score = similarity(input_lower, phrase);
            if score > 0.4 && best.as_ref().map(|(s, _, _, _)| score > *s).unwrap_or(true) {
                best = Some((
                    score,
    ...
    best.map(
        |(confidence, pipeline, context, matched_phrase)| Translation {
            pipeline,
            confidence,
            context,
            matched_phrase,
