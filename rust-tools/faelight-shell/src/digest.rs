...
pub fn render(mem: &SessionMemory, db: &ForestDb, core_root: &str) -> String {
    ...
    if new_commits > 0 && mem.last_commit_count > 0 {
        lines.push(format!("  {} Since last session:",
            "→".bright_cyan()
        ));
        lines.push(format!("    {} {} new commit{}",
            "·".dimmed(),
    ...
    let old_pending: i64 = db.conn.query_row(
    ...
    if old_pending > 0 {
        lines.push(format!("    {} {} pending decision{} older than 7 days",
            "·".yellow(),
    ...
    let low_score: i64 = db.conn.query_row(
        "SELECT COUNT(*) FROM audit_scores WHERE score < 70",
        [], |r| r.get(0)
    ).unwrap_or(0);

    if low_score > 0 {
        lines.push(format!("    {} {} tool{} with audit score < 70",
            "·".yellow(),
