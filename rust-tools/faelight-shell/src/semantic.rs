...
pub fn interpret(input: &str) -> SemanticIntent {
    ...
    match verb {
        ...
        "deploy" => SemanticIntent {
            ...
            confidence: 1.0,
            reversible: true,
            layer2_description: format!("Deploy(Tool(\"{}\")) -- tracked, rollback available", rest),
            layer3_commands: vec![
...
pub fn format_three_layers(si: &SemanticIntent) -> String {
    ...
    let _ = writeln!(out, "  ├─ Layer 2: Semantic Plan ───────────────────────────");
    let _ = writeln!(out, "  │  {}", si.layer2_description);
    let _ = writeln!(out, "  │  category: {}  confidence: {:.0}%  reversible: {}",
        si.category.label(), si.confidence * 100.0,
        if si.reversible { "yes" } else { "no (confirm required)" });
    let _ = writeln!(out, "  ├─ Layer 3: Concrete Execution ──────────────────────");
