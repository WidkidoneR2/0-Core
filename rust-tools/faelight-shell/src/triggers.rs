...
pub fn list(db: &ForestDb) -> Vec<Trigger> {
    let mut stmt = match db.conn.prepare(
        "SELECT id, trigger, action, enabled, fired_count FROM shell_triggers ORDER BY id"
    ) {
        Ok(s) => s,
        Err(_) => return vec![],
    };
    stmt.query_map([], |r| Ok(Trigger {
        id:          r.get(0)?,
        trigger:     r.get(1)?,
        action:      r.get(2)?,
...
pub fn remove(db: &ForestDb, id: i64) -> bool {
    db.conn.execute("DELETE FROM shell_triggers WHERE id = ?1", params![id])
...
fn match_trigger(trigger: &str, ctx: &TriggerContext, db: &ForestDb) -> bool {
    ...
    match parts.as_slice() {
        ...
        ["run", cmd] => {
            ctx.last_command.trim() == cmd.trim()
        }
        _ => false,
    }
}
    ...
    match parts.as_slice() {
        ["run", cmd] => {
            ...
            println!("  {} Trigger {} fired → {}",
                "⚡".yellow(), trigger.trigger.bright_cyan(), cmd.bright_white());
            // Execute the command
        ...
        _ => {
            println!("  {} Unknown trigger action: {}", "✗".bright_red(), trigger.action);
        }
    }
}
