...
fn get_font() -> &'static Font {
    FONT.get_or_init(|| {
        Font::from_bytes(FONT_DATA, FontSettings::default())
            .expect("Failed to load HackNerdFont")
    })
}
            ...
            if alpha > 20 {
                ...
                if px >= 0 && py >= 0 {
                    ...
                    if offset + 3 < canvas.len() {
                        ...
                        canvas[offset+1] = (color[1] as f32 * a + canvas[offset+1] as f32 * ia) as u8;
                        canvas[offset+2] = (color[0] as f32 * a + canvas[offset+2] as f32 * ia) as u8;
...
pub fn draw_notification(
    ...
    // App name — dim, small
    draw_text(canvas, stride, pad, pad, app_name, TEXT_APP, font_size - 1.0);

    // Summary — bright green, medium
    draw_text(canvas, stride, pad, pad + 18, summary, TEXT_SUMMARY, font_size + 1.0);
