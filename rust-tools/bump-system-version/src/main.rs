...
fn preview_changes(
    ...
    println!("\n{}", "Git:".cyan());
    println!(
        "  • Commit message: feat: Release v{} - {}",
        new_version, content.theme
    );
    ...
    println!(
        "  • Health: {}% → {}%",
        stats.old_health, stats.system_health
    );
...
fn execute_updates_safe(
    ...
    fs::copy(
        ...
        paths::core_dir().join("Cargo.toml"),
    )?;
...
fn build_readme_content(
    ...
    let current_readme =
        fs::read_to_string("README.md").or_else(|_| fs::read_to_string("00-meta/README.md"))?;

    let lines: Vec<&str> = current_readme.lines().collect();

    // Find the static section
    let static_start = lines
        .iter()
    ...
    new_readme
        .push_str("![Rust](https://img.shields.io/badge/rust-stable-orange?style=flat-square)\n");
...
fn build_changelog(content: &ReleaseContent, stats: &AutoStats, version: &str) -> Result<String> {
...
fn print_success_summary(version: &str, stats: &AutoStats) -> Result<()> {
    ...
    println!(
        "\n{}",
        format!("🌲 Faelight Forest v{} is live!", version)
            .green()
            .bold()
    );
...
fn get_current_version() -> Result<String> {
    let version =
        fs::read_to_string(paths::version_file()).context("Could not read VERSION file")?;
