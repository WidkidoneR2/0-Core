...
fn gather_data() -> DashboardData {
    ...
    let health_output = Command::new("sh")
        .args([
            "-c",
            "dot-doctor 2>/dev/null | grep -E '✅|⚠️|❌' | head -6",
        ])
        .output()
    ...
    let git_output = Command::new("sh")
        .args([
            "-c",
            &format!(
                "cd {} && git status -s 2>/dev/null | head -5",
                paths::core_dir().display()
            ),
        ])
        .output()
        .map(|o| {
    ...
    let profile = Command::new("sh")
        .args([
            "-c",
            "faelight status 2>/dev/null | grep Profile | awk '{print $2}'",
        ])
        .output()
    ...
    let intents = Command::new("sh")
        .args([
            "-c",
            &format!(
                "ls {}/future/*.md 2>/dev/null | wc -l",
                paths::intents_dir().display()
            ),
        ])
        .output()
        .map(|o| {
    ...
    let packages = Command::new("sh")
        .args([
            "-c",
            &format!(
                "cat {}/packages/pkglist.txt 2>/dev/null | wc -l",
                paths::core_dir().display()
            ),
        ])
        .output()
    ...
    let snapshots = Command::new("sh")
        .args([
            "-c",
            "snapper -c root list 2>/dev/null | tail -1 | awk '{print $7, $8}'",
        ])
        .output()
        .map(|o| {
