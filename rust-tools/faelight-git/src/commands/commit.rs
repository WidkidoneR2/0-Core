    ...
    // ── v4 Risk Assessment ─────────────────────────────────
    {
        ...
        if let Ok(conn) = rusqlite::Connection::open(&db_path) {
            ...
            let recent: i64 = conn
                .query_row(
                    "SELECT COUNT(*) FROM commit_patterns WHERE timestamp > ?1",
                    rusqlite::params![ts - 3600],
                    |r| r.get(0),
                )
                .unwrap_or(0);
            if recent >= 8 {
                println!(
                    "  {} high velocity: {} commits in last hour -- verify before pushing",
                    "⚠️ ".yellow(),
                    recent.to_string().bright_yellow()
                );
            }
            // Health warning
            ...
            .output()
        {
            ...
            if let Some(last) = stat.lines().last() {
                ...
                if total_lines >= 800 && file_count >= 10 {
                    println!(
                        "  {} large change: {} lines across {} files -- consider splitting",
                        "⚠️ ".yellow(),
