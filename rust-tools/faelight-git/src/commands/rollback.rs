...
fn compute_risk_score(hash: &str) -> u32 {
    ...
    let diff_stat = Command::new("git")
        ...
        .unwrap_or_default();
    let files: u32 = diff_stat.lines().filter(|l| l.contains("|")).count() as u32;
    (files * 2).min(100)
}
fn show_rollback_list(dry_run: bool) -> Result<()> {
    ...
    println!(
        "{}",
        "🔄 fg rollback -- Last 10 commits".bright_cyan().bold()
    );
    ...
    for (i, (hash, msg, date)) in commits.iter().enumerate() {
        ...
        let risk_color = if risk < 20 {
            short_hash.bright_green().to_string()
        } else if risk < 50 {
            short_hash.bright_yellow().to_string()
        } else {
            short_hash.bright_red().to_string()
        };
        let risk_label = if risk < 20 {
            "low".bright_green()
        } else if risk < 50 {
            "med".bright_yellow()
        } else {
    ...
    if dry_run {
        println!(
            "  {} dry-run mode -- select a commit to see rollback plan",
            "○".dimmed()
        );
        return Ok(());
    }
...
fn do_rollback(hash: &str, dry_run: bool) -> Result<()> {
    ...
    if status.success() {
        println!(
            "  {} reverted -- changes staged, review and commit",
            "✅".green()
        );
        println!(
            "  {} run: fg commit to complete rollback",
            "→".bright_cyan()
        );
    } else {
        println!(
            "  {} revert failed -- resolve conflicts manually",
            "✗".bright_red()
        );
    }
    println!();
