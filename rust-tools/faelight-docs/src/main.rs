...
fn cmd_readme(dry_run: bool) {
    ...
    // PATCH strategy — only update the timestamp line, never replace content
    let static_section = &content[boundary_pos..];
    ...
    let updated_static =
        if let Some(start) = static_section.find("*Auto-generated by faelight-docs") {
            let end = static_section[start..]
                .find('\n')
                .map(|i| start + i)
                .unwrap_or(static_section.len());
            format!(
                "{}{}{}",
                &static_section[..start],
                timestamp,
                &static_section[end..]
            )
        } else {
            format!("{}\n{}\n", static_section.trim_end(), timestamp)
        };

    let new_content = format!("{}{}", &content[..boundary_pos], updated_static);
    ...
    if dry_run {
        println!(
            "  {} README would update timestamp — tools: {}  intents: {}  version: {}",
            "→".bright_cyan(),
            state.tool_count,
        ...
        match std::fs::write(&readme_path, &new_content) {
            Ok(_) => println!("  {} README timestamp updated", "✅".normal()),
...
fn generate_static_section(state: &ForestState) -> String {
    ...
    let changelog = std::fs::read_to_string(root.join("00-meta/CHANGELOG.md")).unwrap_or_default();
    let journey_entries: Vec<String> = changelog
        .lines()
