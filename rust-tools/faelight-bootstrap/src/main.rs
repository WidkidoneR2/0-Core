...
fn run_installation() {
    ...
    if confirm("  Install ~40 system packages?") {
        ...
        println!(
            "{}",
            "  ⚠️  Skipping dependencies (may cause issues)".yellow()
        );
    }
...
fn print_banner() {
    ...
    println!();
    println!(
        "  {} Philosophy-driven Linux desktop environment",
        "▸".green()
    );
...
fn print_success() {
    ...
    println!(
        "{}",
        "✅ Faelight Forest Installation Complete!".green().bold()
    );
    println!("{}", "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".green());
    println!();
...
fn check_not_root() {
    ...
    if env::var("USER").unwrap_or_default() == "root" {
        ...
        eprintln!();
        eprintln!(
            "{} Don't run as root (sudo used internally)",
            "  Error:".red().bold()
        );
        exit(1);
    }
...
fn build_rust_workspace(core_path: &str) {
    ...
    match status {
        ...
        _ => {
            ...
            eprintln!();
            eprintln!(
                "{} Build failed - some tools may be unavailable",
                "  Warning:".yellow().bold()
            );
        }
    }
...
fn run_health_check(core_path: &str) {
    ...
    if Path::new(&doctor).exists() {
        ...
        Command::new(&doctor).status().ok();

        println!();
    } else {
        println!(
            "  {} Health check unavailable (dot-doctor not built)",
            "⚠️".yellow()
        );
    }
}
