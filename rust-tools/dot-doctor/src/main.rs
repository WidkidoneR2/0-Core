...
struct Cli {
    ...
    #[arg(long)]
    history: bool,
    /// Show fixes without applying (dry-run mode)
    #[arg(long, name = "fix-dry-run")]
    fix_dry_run: bool,
    ...
    /// Skip specific check(s)
    #[arg(long)]
    skip: Option<Vec<String>>,

    /// Minimum health threshold (0-100)
    #[arg(long, name = "min-health")]
...
fn main() {
    ...
    for check in CHECKS {
        ...
        if let Some(ref only) = cli.check {
            ...
            if let Some(ref skip_list) = cli.skip {
                if skip_list.iter().any(|s| s == check.id) {
                    continue;
                }
            }
        }
    ...
    if let Some(min_health) = cli.min_health {
        if health_percent < min_health as u32 {
            eprintln!(
                "❌ Health {}% is below minimum threshold {}%",
                health_percent, min_health
            );
            std::process::exit(1);
        }
    }
