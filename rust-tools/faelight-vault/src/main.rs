...
fn main() {
    ...
    match cmd {
        "--health" | "health" => {
            println!("faelight-vault v1.0.0 — healthy");
            process::exit(0);
        }
        "init" => cmd_init(),
        "add" => cmd_add(&args),
...
fn cmd_add(args: &[String]) {
    ...
    let secret = if answer == "n" {
        ...
        generate_secret(cred_type)
    };
    if secret.is_empty() {
        eprintln!("  {} No secret", "✗".bright_red());
        process::exit(1);
    }
    match store::add_credential(&master, &name, cred_type, &secret) {
        Ok(_) => {
            let score = health::score(&secret, 0);
            display::print_added(&name, cred_type, score);
        }
        Err(e) => {
            eprintln!("  {} {}", "✗".bright_red(), e);
            process::exit(1);
...
fn cmd_get(args: &[String]) {
    ...
    match store::get_credential(&master, &name) {
        ...
        Ok(None) => {
            eprintln!("  {} Not found: {}", "✗".bright_red(), name);
            process::exit(1);
        }
        Err(e) => {
            eprintln!("  {} {}", "✗".bright_red(), e);
...
fn cmd_list(args: &[String]) {
    ...
    match store::list_credentials(&master) {
        Ok(entries) => {
            display::print_banner();
            display::print_list(&entries, filter);
        }
        Err(e) => {
            eprintln!("  {} {}", "✗".bright_red(), e);
            process::exit(1);
    ...
    match store::get_credential(&master, &name) {
        Ok(Some(e)) => {
            ...
            match store::update_credential(&master, &name, &new_secret) {
                Ok(_) => {
                    println!("  {} {} rotated", "🔄".normal(), name.bright_white());
                    display::print_health_bar(health::score(&new_secret, 0));
                }
...
fn cmd_unlock(args: &[String]) {
    ...
    match store::validate_master(&master) {
        Ok(true) => {
            store::write_session_cache(&master, ttl);
            println!(
                "  {} Vault unlocked for {}",
                "🔓".normal(),
                ttl.bright_green()
        ...
        Ok(false) => {
            eprintln!("  {} Invalid master password", "✗".bright_red());
            process::exit(1);
        }
        Err(e) => eprintln!("  {} {}", "✗".bright_red(), e),
    }
...
fn cmd_help() {
    ...
    for (c, d) in &cmds {
        println!("  {:26} {}", c.bright_cyan(), d.dimmed());
    }
    println!();
}
...
fn generate_secret(cred_type: &str) -> String {
    ...
    std::process::Command::new("faelight-gen")
        .args(args)
        .output()
        .ok()
