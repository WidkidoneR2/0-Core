{ config, pkgs, lib, self, system, inputs, ... }:
{
  ...
  services.greetd = {
    ...
    settings.default_session = {
      command = "${pkgs.tuigreet}/bin/tuigreet --time --remember --remember-session --sessions /etc/greetd/sessions --asterisks --greeting \"🌲 Faelight Forest\" --theme \"border=green;text=green;prompt=green;time=green;action=blue;button=green;container=black;input=green\"";
      user = "greeter";
    };
  };
