{ config, pkgs, lib, self, system, inputs, ... }:
{
  ...
  services.greetd = {
    ...
    settings.default_session = {
      command = "${pkgs.tuigreet}/bin/tuigreet --time --remember --remember-user-session --remember-session --sessions /etc/greetd/sessions --asterisks --kb-sessions 2 --greeting \"🌲 Faelight Forest\" --theme \"border=green;text=green;prompt=green;time=green;action=blue;button=green;container=black;input=green\"";
      user = "greeter";
    };
  };
