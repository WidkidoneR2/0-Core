...
- [~] cargo-nextest runs test suite faster than cargo test <!-- INT-130: 2026-07-10 -- NOT YET MET. cargo-nextest 0.9.136 is present + functional (`cargo nextest list` drove the test-binary build, correct diagnostics), BUT the literal 'faster than cargo test' claim is UNMEASURED: the workspace will not compile in-shell until udev + xkbcommon land (INT-137). This gate stays OPEN and reopens for a real timed comparison once 137 closes. Deferred, not done. -->
