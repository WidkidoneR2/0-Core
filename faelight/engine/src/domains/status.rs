...
pub fn run(ctx: &AppContext) -> CoreResult<()> {
    ...
    let observations = crate::domains::friday::reasoning::reason(ctx).unwrap_or_default();
    let anomalies: Vec<_> = observations
        .iter()
        .filter(|(_, _, k)| k == "anomaly")
        .collect();

    // ── Contradictions ──
    let contradictions: i64 = db
        .query_row(
    ...
    if contradictions > 0 {
        println!(
            "  {} {} active contradiction(s) -- run: core friday-arch run",
            "⚠".bright_yellow(),
            contradictions
        );
        println!();
    ...
    if intent_count >= 4 {
        ...
        println!(
            "    {} Consider committing and resting -- {} deploys today",
            "→".bright_cyan(),
            deploys_today
        );
    } else {
