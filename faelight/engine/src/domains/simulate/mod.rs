...
pub fn scenario(ctx: &AppContext, description: &str) -> CoreResult<()> {
    ...
    if context.health_score < 90 {
        signals.push("health significantly degraded");
    }
    ...
    if signals.is_empty() {
        println!(
            "    {} No risk signals — favorable conditions",
            "✅".green()
        );
    } else {
        for s in &signals {
    ...
    if similar.is_empty() {
        println!(
            "    {} No historical decisions in similar context",
            "○".dimmed()
        );
        println!(
            "    {} Build decision history with {}",
            "→".dimmed(),
        ...
        if successes > 0 {
            ...
            println!(
                "      Checkpoint created before {} of {} successes",
                successes, successes
            );
        }
    ...
    let combined_label = if combined_risk < 0.3 {
        "low"
    ...
    let risk_display = match combined_label {
        "low" => format!("{:.2} ({})", combined_risk, combined_label)
            .green()
            .to_string(),
        "moderate" => format!("{:.2} ({})", combined_risk, combined_label)
            .yellow()
            .to_string(),
        _ => format!("{:.2} ({})", combined_risk, combined_label)
            .red()
            .to_string(),
    };
    ...
    if context.git_churn_level > 1 {
        println!(
            ...
            "→".dimmed(),
            "cpc pre-scenario".bright_cyan()
        );
    }
    if context.health_score < 95 {
        println!(
            "    {} Improve health first: {}",
            "→".dimmed(),
            "d".bright_cyan()
        );
    }
