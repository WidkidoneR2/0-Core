...
pub fn run(ctx: &AppContext) -> CoreResult<()> {
    ...
    let threshold = if accuracy >= 85.0 {
        0.80
    ...
    let calibration_msg = if accuracy >= 85.0 {
        "well-calibrated -- speaking at 80% threshold"
    } else if accuracy >= 75.0 {
        "adequate -- speaking at 85% threshold"
    } else {
        "needs improvement -- speaking at 90% threshold"
    };
    ...
    if total > 10 {
        if accuracy >= 80.0 {
            println!(
                "  {} Confidence well-calibrated ({:.0}%)",
                "✅".green(),
                accuracy
            );
        } else {
            println!(
                "  {} Needs recalibration ({:.0}% accuracy)",
                "⚠️ ".yellow(),
                accuracy
            );
        }
        ...
        println!(
            "  {} Not enough data to calibrate ({} resolved)",
            "ℹ️ ".cyan(),
            total
        );
    }
