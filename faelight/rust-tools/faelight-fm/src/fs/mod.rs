...
use crate::types::{FlatNode, GitStatus, TreeNode};
...
fn forest_context(path: &PathBuf) -> String {
    ...
    let mut ctx = vec![];
    if p.contains("/intents/") {
        ctx.push("📋 Intent directory");
    }
    if p.contains("/rust-tools/") {
        ctx.push("🦀 Rust tool source");
    }
    if p.contains("/engine/") {
        ctx.push("⚙️  Core engine");
    }
    if p.contains("/pkgs/") {
        ctx.push("📦 Nix derivations");
    }
    if p.contains("/modules/") {
        ctx.push("🔧 NixOS module");
    }
    if p.contains("/config/") {
        ctx.push("⚙️  Forest config");
    }
    if p.contains("/nix/store/") {
        ctx.push("❄️  Nix store");
    }
    ctx.join("\n")
}
