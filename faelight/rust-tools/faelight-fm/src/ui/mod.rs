...
fn render_header(
    ...
    let title_style = Style::default()
        ...
        .bg(BG);
    let dim_seg = Style::default()
        .fg(if active { DIM_GREEN } else { DIM_GRAY })
        .bg(BG);
    let cur_seg = Style::default()
        .fg(if active { CYAN } else { GRAY })
        .add_modifier(Modifier::BOLD)
        .bg(BG);
    ...
    for (i, seg) in segs[start..].iter().enumerate() {
        ...
        spans.push(Span::styled(
            seg.to_string(),
            if is_last { cur_seg } else { dim_seg },
        ));
        if !is_last {
            spans.push(Span::styled(" › ", sep_style));
        }
        ...
        .map(|(display_idx, &real_idx)| {
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("{} ", icon),
                    Style::default()
                        .fg(if node.is_dir { CYAN } else { GRAY })
                        .bg(bg),
                ),
...
fn render_preview(f: &mut ratatui::Frame, area: ratatui::layout::Rect, preview: &str) {
