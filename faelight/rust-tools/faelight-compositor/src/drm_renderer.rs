...
pub fn init_gbm_pipeline(
    ...
    let gbm: GbmDevice<DrmDeviceFd> =
        GbmDevice::new(drm_fd.clone()).map_err(|e| format!("GBM device: {e}"))?;
    ...
    let egl_context = EGLContext::new(&egl_display).map_err(|e| format!("EGL context: {e}"))?;

    let renderer =
        unsafe { GlesRenderer::new(egl_context).map_err(|e| format!("GLES renderer: {e}"))? };
...
pub fn render_frame(pipeline: &mut GbmRenderPipeline, state: &mut FaelightCompositor) {
    ...
    match pipeline
        .compositor
        .render_frame::<GlesRenderer, WaylandSurfaceRenderElement<GlesRenderer>>(
            &mut pipeline.renderer,
            &elements,
            CLEAR_COLOR,
            FrameFlags::DEFAULT,
...
pub fn init_dmabuf(pipeline: &mut GbmRenderPipeline, state: &mut FaelightCompositor) {
    ...
    let dmabuf_formats = pipeline
        .renderer
        .egl_context()
        .dmabuf_render_formats()
        .clone();
    let node =
        smithay::backend::drm::DrmNode::from_file(pipeline.compositor.surface().device_fd()).ok();
