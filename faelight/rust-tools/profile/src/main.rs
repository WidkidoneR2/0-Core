...
fn error(msg: &str) -> ! {
    ...
    if msg.contains("not found") || msg.contains("doesn't exist") {
        eprintln!("💡 Run 'profile list' to see available profiles");
    } else if msg.contains("script") {
        eprintln!("💡 Check activation/deactivation scripts in profile directory");
    } else if msg.contains("Usage") {
        eprintln!("💡 Run 'profile help' for command details");
    }
    process::exit(1);
}
