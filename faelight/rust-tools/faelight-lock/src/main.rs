...
impl LockApp {
    ...
    fn render_all(&mut self, _qh: &QueueHandle<Self>) {
        ...
        let commits =
            std::fs::read_to_string("/etc/faelight/COMMITS").unwrap_or_else(|_| "?".to_string());
        let w_list: Vec<(u32, u32)> = self.surfaces.iter().map(|s| (s.width, s.height)).collect();
        for (i, &(w, h)) in w_list.iter().enumerate() {
            if !self.surfaces[i].configured || w == 0 || h == 0 {
                continue;
            }
            ...
            // Version + commits
            let info = format!("{}  \u{00b7}  {} commits", version.trim(), commits.trim());
            draw_text(canvas, w, cx - 100, cy - 58, &info, 16.0, &DIM, &self.font);
            // Password field
            ...
            if let Some(ref err) = error {
                draw_text(canvas, w, cx - 100, cy + 60, err, 16.0, &RED, &self.font);
            }
            // Hint
            draw_text(
                canvas,
