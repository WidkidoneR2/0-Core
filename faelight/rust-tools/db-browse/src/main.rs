...
fn table_icon(name: &str) -> &'static str {
    ...
    if n.contains("event") || n.contains("audit") {
        return "📋";
    }
    if n.contains("deploy") {
        return "🚀";
    }
    if n.contains("failure") || n.contains("integrity") || n.contains("security") {
        return "🔒";
    }
    if n.contains("history") || n.contains("snapshot") || n.contains("checkpoint") {
        return "📊";
    }
    if n.contains("shell") || n.contains("alias") {
        return "⚙️ ";
    }
    "  "
}
...
fn draw_table_list(f: &mut ratatui::Frame, app: &mut App, area: Rect) {
    let border_color = if app.active_panel == Panel::Tables {
        GREEN
    } else {
        DIM
    };
...
fn draw_data_panel(f: &mut ratatui::Frame, app: &mut App, area: Rect) {
    let border_color = if app.active_panel == Panel::Data {
        GREEN
    } else {
        DIM
    };
    ...
    for i in app.col_offset..app.data_headers.len() {
        ...
        if used + w > available_width && !visible_cols.is_empty() {
            break;
        }
        visible_cols.push(i);
        used += w;
                ...
                Mode::Normal => match key.code {
                    ...
                    KeyCode::Char('l') | KeyCode::Right => {
                        if app.active_panel == Panel::Data {
                            app.col_offset =
                                (app.col_offset + 1).min(app.data_headers.len().saturating_sub(1));
                        } else {
                    ...
                    KeyCode::Char('h') | KeyCode::Left => match app.active_panel {
                        Panel::Data => {
                            app.col_offset = app.col_offset.saturating_sub(1);
                        }
                        Panel::Tables => {
                            if let Some(idx) =
                                app.tables.iter().position(|(n, _)| n == "shell_history")
                            {
                                app.table_state.select(Some(idx));
                                app.load_table(idx);
                            }
                    ...
                    KeyCode::Char('f') => {
                        if let Some(idx) =
                            app.tables.iter().position(|(n, _)| n == "friday_patterns")
                        {
                            app.table_state.select(Some(idx));
                            app.load_table(idx);
                            app.active_panel = Panel::Data;
                    ...
                    KeyCode::Char('a') => {
                        if let Some(idx) =
                            app.tables.iter().position(|(n, _)| n == "friday_attention")
                        {
                            app.table_state.select(Some(idx));
                            app.load_table(idx);
                            app.active_panel = Panel::Data;
                    ...
                    KeyCode::Char('c') => {
                        if let Some(idx) =
                            app.tables.iter().position(|(n, _)| n == "intent_commits")
                        {
                            app.table_state.select(Some(idx));
                            app.load_table(idx);
                            app.active_panel = Panel::Data;
                    ...
                    KeyCode::Char('s') => {
                        app.mode = if app.mode == Mode::Schema {
                            Mode::Normal
                        } else {
                            Mode::Schema
                        };
