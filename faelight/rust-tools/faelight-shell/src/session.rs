...
fn sc_bold(r: u8, g: u8, b: u8, text: &str) -> String {
    format!("\x1b[1m\x1b[38;2;{};{};{}m{}\x1b[0m", r, g, b, text)
}
fn sc_dim(r: u8, g: u8, b: u8, text: &str) -> String {
...
pub fn render(mem: &SessionMemory, core_root: &str, db: &crate::db::ForestDb) -> String {
    ...
    let welcome = match &mode {
        ...
        ShellMode::Milestone => format!(
            "  {} {}",
            sc_bold(255, 230, 50, "✦"),
            sc_bold(255, 215, 0, "All intents complete. What grows next?")
        ),
        ShellMode::Focused => {
    ...
    if !intents.is_empty() {
        ...
        lines.push(format!(
            "  {} {}",
            sc_dim(120, 200, 100, "Working on:"),
            sc_bold(50, 220, 255, &intent_str)
        ));
    }
    ...
    if new > 0 && mem.last_commit_count > 0 {
        lines.push(format!(
            "  {} {} {}",
            sc_dim(120, 200, 100, "Since last session:"),
            sc_bold(57, 255, 20, &new.to_string()),
            if new == 1 {
                sc_dim(120, 200, 100, "new commit")
            } else {
                sc_dim(120, 200, 100, "new commits")
            }
        ));
    }
