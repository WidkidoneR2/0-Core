...
fn insert_insight(
    ...
    if importance < 0.70 || confidence < 0.60 {
        return;
    }
...
fn detect_signals(conn: &Connection) {
    ...
    if recent_failures >= 4 {
        insert_insight(
            ...
            "failure-loop",
            &format!(
                "{} failures in 5 minutes — something is broken. Check the error above carefully.",
                recent_failures
            ),
            0.90,
            0.85,
        ...
        insert_insight(
            conn,
            "failure-pattern-forming",
            &format!(
                "{} failures in 5 minutes — pattern forming. What changed recently?",
                recent_failures
            ),
            0.72,
            0.65,
...
fn auto_verify_predictions(conn: &Connection) {
    ...
    let preds: Vec<(i64, String, String)> = {
        let mut stmt = conn
            ...
            .unwrap();
        stmt.query_map(params![now - 86400], |r| {
...
fn main() {
  ...
  🧠 zero-insightd"
            ...
            println!(
                "  {:<20} {}",
                "events observed:".dimmed(),
                event_count.to_string().bright_white()
            );
            println!(
                "  {:<20} {}",
                "pending insights:".dimmed(),
                insight_count.to_string().bright_cyan()
            );
        ...
        Cmd::Start => {
            println!("  🧠 zero-insightd starting (30s poll interval)...");
            let conn = open_db().expect("db error");
  ...
  {}",
            ...
            for (kind, domain, detail, ts) in &rows {
                let time = chrono::DateTime::from_timestamp(*ts, 0)
                    ...
                    .unwrap_or_default();
                let k = if kind.contains("Succeeded") {
                    kind.bright_green()
                } else {
                    kind.bright_red()
                };
