...
fn check_unfailable_checks(root: &Path) -> Checked<Vec<Finding>> {
    let dir = root.join("zero/engine/src/domains/doctor");
    if !dir.exists() {
...
fn check_dangling_intent_citations(root: &Path) -> Vec<Finding> {
    ...
    let intents = root.join("zero/intents");
...
fn check_registry_orphans(root: &Path) -> Checked<Vec<Finding>> {
    // WAS root.join(registry/tools.toml) -- the file is at zero/registry/tools.toml,
