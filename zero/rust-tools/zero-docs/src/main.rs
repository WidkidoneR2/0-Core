...
fn cmd_command_guide(dry_run: bool) {
    println!("  Generating COMMAND-GUIDE.md...");
    ...
    let mut md = format!(
        "# Project 0 -- Core Command Reference\n\n**Version:** {}  \n**Generated:** {}  \n**Domains:** {}  \n**Source:** Auto-generated by zero-docs\n\n---\n\n",
...
fn gather_state() -> LiveState {
    ...
    let changelog =
        std::fs::read_to_string(root.join("zero/meta/CHANGELOG.md")).unwrap_or_default();
    let theme = changelog
        .lines()
    ...
    let tool_count = match std::fs::read_to_string(&tools_path) {
        ...
        //                 elsewhere (zero/engine/, and novashell's binary).
        //
    ...
    // Read live health — prefer ~/.cache/zero/health-status, fall back to state.db cache
    let health = {
    ...
    // was engine/src/domains and the engine has been at zero/engine since INT-061
...
fn cmd_readme(dry_run: bool) {
    ...
    let timestamp = format!(
        "*Auto-generated by zero-docs v{} — last sync: {}*",
        VERSION,
        chrono::Local::now().format("%Y-%m-%d %H:%M")
    );
    body = if let Some(start) = body.find("*Auto-generated by zero-docs") {
        let end = body[start..]
            .find('\n')
            .map(|i| start + i)
            .unwrap_or(body.len());
...
fn strip_timestamp(s: &str) -> String {
    s.lines()
        .filter(|l| !l.starts_with("*Auto-generated by zero-docs"))
        .collect::<Vec<_>>()
        .join("\n")
}
...
fn cmd_status() {
    ...
    println!();
    println!("  {} zero-docs status", "📋".normal());
    println!("{}", "  ─────────────────────────────────────".dimmed());
    println!(
        "  {}  v{} — {}",
        "Version:".dimmed(),
    ...
    println!(
        ...
        if readme_ok {
            ...
            "⚠  numbers outdated — run: zero-docs readme"
                .yellow()
                .to_string()
        }
    );
...
fn cmd_help() {
    println!();
    println!("  {} {}", "📋".normal(), "zero-docs".bright_green().bold());
    println!("  {}", "Living Documentation Engine".dimmed());
    println!("{}", "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".dimmed());
    println!();
    let cmds = [
        ("sync", "Update all docs from the current state"),
        ("check", "Show what is out of date (dry run)"),
