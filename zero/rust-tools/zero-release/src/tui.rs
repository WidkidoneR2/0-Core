...
impl ReleaseTui {
    ...
    fn draw(&self, f: &mut ratatui::Frame) {
        ...
        let theme_widget =
            Paragraph::new(Span::styled(theme_display, theme_style)).block(theme_block);
        ...
        // Intents shipped
        let intent_items: Vec<ListItem> = self
            .data
            .intents
            .iter()
            .map(|i| {
        ...
        let stats_items = vec![
            ...
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("{}", self.stats.total_commits),
                    Style::default().fg(FG).add_modifier(Modifier::BOLD),
                ),
            ])),
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("{} deployed", self.stats.tools_deployed),
                    Style::default().fg(FG),
                ),
            ])),
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("{} complete", self.stats.intents_complete),
                    Style::default().fg(FG),
                ),
            ])),
            ListItem::new(Line::from(vec![
                ...
                Span::styled(
                    format!("{} commits", self.data.total_commits),
        ...
        if !self.data.features.is_empty() {
            commit_items.push(ListItem::new(Line::from(vec![Span::styled(
                format!("  ✨ Features    {}", self.data.features.len()),
                Style::default().fg(ACCENT).add_modifier(Modifier::BOLD),
            )])));
        ...
        if !self.data.fixes.is_empty() {
            commit_items.push(ListItem::new(Line::from(vec![Span::styled(
                format!("  🔧 Fixes       {}", self.data.fixes.len()),
                Style::default().fg(YELLOW).add_modifier(Modifier::BOLD),
            )])));
        ...
        if !self.data.internal.is_empty() {
            commit_items.push(ListItem::new(Line::from(vec![Span::styled(
                format!("  🔩 Internal    {}", self.data.internal.len()),
        ...
        if self.state == State::Confirm {
            ...
            let hint = Paragraph::new(vec![
                ...
                Line::from(vec![Span::styled(
                    "  · README.md dynamic section updated",
                    Style::default().fg(FG),
                )]),
                Line::from(vec![Span::styled(
                    "  · meta/VERSION bumped",
        ...
        let footer_text = match self.state {
            ...
            State::Done => vec![Span::styled(
                "  ✅ Release published! Press any key to exit.",
                Style::default().fg(ACCENT).add_modifier(Modifier::BOLD),
            )],
            _ => vec![],
        };
        let footer = Paragraph::new(Line::from(footer_text)).style(Style::default().fg(DIM).bg(BG));
...
"#,
        ...
        let readme_meta_path = core_root.join("zero/meta/README.md");
        ...
        if let Err(e) = crate::readme::update_readme(
            ...
            self.log
                .push("✅ README.md dynamic section updated".to_string());
        }
        ...
        // 3. Update CHANGELOG
        let changelog_path = core_root.join("zero/meta/CHANGELOG.md");
        ...
        // 5. Git commit
        Command::new("git")
            .args(["-C", core_root.to_str().unwrap_or("."), "add", "-A"])
            .output()?;
        Command::new("git")
            .args([
                "-C",
        ...
        Command::new("git")
            .args([
                ...
                "-m",
                &format!("v{} — {}", self.version, self.theme),
            ])
            .output()?;
