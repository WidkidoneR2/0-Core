...
fn eval_health_advisory(ctx: &AppContext) -> Option<Reaction> {
    ...
    if recent.len() >= 2 && below >= 2 {
        ...
        Some(Reaction {
            ...
            message: format!(
                "Health below 95% in {}/{} recent runs (avg: {}%)",
                below,
                recent.len(),
                avg
            ),
...
fn eval_intent_overflow(ctx: &AppContext) -> Option<Reaction> {
    ...
    if count > 2 {
        Some(Reaction {
            ...
            message: format!(
                "{} intents in-progress — focus narrows effectiveness",
                count
            ),
...
fn eval_forecast_declining(ctx: &AppContext) -> Option<Reaction> {
    ...
    if trend < -1.5 {
        Some(Reaction {
            ...
            message: format!(
                "Forecast trend {:.1} — health declining before it hits",
                trend
            ),
...
fn goal_context_for(rule_id: &str, goals: &[GoalContext]) -> Option<String> {
    ...
    let keywords: &[&str] = match rule_id {
        ...
        r if r.starts_with("security.") => &["security", "audit", "hardening"],
        "intent.overflow" => &["intent", "goal", "focus", "retire", "tool"],
        "checkpoint.stale" => &["resilience", "backup", "checkpoint", "snapshot"],
        _ => &[],
    };
...
fn toggle_rule(_ctx: &AppContext, id: &str, enabled: bool) -> CoreResult<()> {
    ...
    if found {
        ...
        println!(
            "  {} rule '{}' not found in runtime/reactions/",
            "✗".bright_red(),
            id
        );
...
pub fn rules_list(ctx: &AppContext) -> CoreResult<()> {
                ...
                if let Some(custom_rules) = parsed.rule {
                    if !custom_rules.is_empty() {
                        ...
                        for r in &custom_rules {
                            ...
                            println!("  🔧 {}  {}", r.id.bright_white(), status,);
    ...
    println!("{}", "━".repeat(52).dimmed());
    println!(
        "  {} core react enable/disable <id>  — toggle a rule",
        "hint:".dimmed()
    );
    Ok(())
}
...
pub fn bounds(ctx: &AppContext) -> CoreResult<()> {
    ...
    let gate_status = |threshold: u32, _label: &str| {
        if health >= threshold {
            format!("✅ OPEN  (health {}% ≥ {}%)", health, threshold)
                .green()
                .to_string()
        } else {
            format!("🔒 CLOSED (health {}% < {}%)", health, threshold)
                .yellow()
                .to_string()
        }
    };
    ...
    println!();
    println!(
        "  {} Stability rules  (health.*, forecast.*)  {}",
        "🔴".normal(),
        gate_status(0, "0")
    );
    ...
    println!();
    println!(
        "  {} Maintenance rules (security.*, intent.*)  {}",
        "🟡".normal(),
        gate_status(80, "80")
    );
    ...
    println!(
        "  {} Rule 1: Reactions bounded by accepted v9 goals",
        "🛡️ ".normal()
    );
    println!(
        "  {} Rule 2: Reactions propose, never execute",
        "🛡️ ".normal()
    );
    println!(
        "  {} Rule 3: Stability gates enforced in code",
        "🛡️ ".normal()
    );
    println!(
        "  {} Rule 4: Discipline (cooldown/decay) non-optional",
        "🛡️ ".normal()
    );
    println!();
...
pub fn coalesce(ctx: &AppContext) -> CoreResult<()> {
    ...
    if coalesce_groups.is_empty() {
        println!(
            "  {} No coalescing candidates — no batched signals pending",
            "○".dimmed()
        );
    } else {
        ...
        println!(
            "  {} These signals would surface together in one notification",
            "hint:".dimmed()
        );
    }
        ...
        let decay_status = match last {
            Some(ts) => {
                ...
                if elapsed_h > decay_h {
                    format!("⏱  decayed ({:.1}h > {:.0}h)", elapsed_h, decay_h)
                        .dimmed()
                        .to_string()
                } else {
                    format!("✅ active  ({:.1}h of {:.0}h)", elapsed_h, decay_h)
                        .green()
                        .to_string()
                }
            }
    ...
    for rule in &all_rules {
        ...
        let status = match last_fired {
            Some(ts) if (now - ts) < rule.cooldown_s => {
                ...
                format!("🔵 cooldown {}m", remaining / 60)
                    .dimmed()
                    .to_string()
            }
            Some(ts) => {
                let ago = (now - ts) / 60;
                format!("✅ ready  (last fired {}m ago)", ago)
                    .green()
                    .to_string()
            }
        ...
        println!("  {} {}  {}", priority_icon, rule.id.bright_white(), status,);
        println!(
            "    {}  cooldown: {}m",
...
pub fn explain(ctx: &AppContext, id: &str) -> CoreResult<()> {
    ...
    let row: Option<(String, i64, String, String)> = ctx
        .runtime
        .db
        .query_row(
        ...
        Some((rule_id, ts, message, context)) => {
            ...
            if let Some(r) = rule {
                ...
                println!(
                    "  {}  {}m",
                    "Cooldown:".dimmed(),
                    (r.cooldown_s / 60).to_string().dimmed()
                );
...
pub fn discipline(ctx: &AppContext) -> CoreResult<()> {
    ...
    if !any_active {
        println!(
            "  {} No active cooldowns — all rules ready to fire",
            "✅".green()
        );
    }
