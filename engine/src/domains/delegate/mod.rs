...
#[allow(dead_code)]
pub enum Capability {
    RestartService    { name: String, allowed_services: Vec<String>, max_per_hour: u8 },
    CreateCheckpoint  { tag: String, max_per_session: u8 },
    NotifyUser        { message: String },
    RunDiagnostic     { check_name: String, read_only: bool },
...
pub enum RollbackAction {
    ...
    RestoreFile     { path: std::path::PathBuf, backup: std::path::PathBuf },
    RevertDb        { checkpoint: String },
    CreateCheckpoint { tag: String },
...
impl DelegateAccuracy {
    pub fn activation_ready(&self) -> bool {
        self.action_match >= 0.85
            && self.outcome_success >= 0.80
            && self.calibration_error <= 0.10
    }
    pub fn from_db(ctx: &AppContext) -> Self {
        let total: i64 = ctx.runtime.db.query_row(
            "SELECT COUNT(*) FROM delegate_counterfactuals", [], |r| r.get(0)
        ).unwrap_or(0);
        if total == 0 {
            return Self { action_match: 0.0, outcome_success: 0.0, calibration_error: 1.0 };
        }
        let matches: i64 = ctx.runtime.db.query_row(
            "SELECT COUNT(*) FROM delegate_counterfactuals WHERE action_match = 1", [], |r| r.get(0)
        ).unwrap_or(0);
        let successes: i64 = ctx.runtime.db.query_row(
            "SELECT COUNT(*) FROM delegate_counterfactuals WHERE outcome_matched = 1", [], |r| r.get(0)
        ).unwrap_or(0);
        // calibration_error = avg |predicted_confidence - actual_success_rate|
        let calib: f64 = ctx.runtime.db.query_row(
            "SELECT AVG(ABS(predicted_confidence - CAST(outcome_matched AS REAL)))
             FROM delegate_counterfactuals", [], |r| r.get::<_, f64>(0)
        ).unwrap_or(1.0);
        Self {
...
pub fn check_hard_boundaries(action: &str) -> Option<&'static str> {
    ...
    if action_lower.contains("rm ") || action_lower.contains("delete") || action_lower.contains("remove") {
        return Some("file deletion is a hard boundary — never delegated");
    }
    if action_lower.contains("state.db") || action_lower.contains("engine/src") || action_lower.contains("scripts/core") {
...
fn seed_contracts(ctx: &AppContext) {
    ...
    let contracts = vec![
        ("auto-checkpoint",  "LOW",      0.85, 1, "core checkpoint restore {id}", "once_per_session", "on-failure"),
        ("restart-service",  "MEDIUM",   0.80, 1, "systemctl --user start {name}", "once_per_hour",   "always"),
        ("clear-cache",      "LOW",      0.85, 0, "",                              "once_per_day",    "on-failure"),
...
pub fn simulate(ctx: &AppContext, action: &str) -> CoreResult<()> {
    ...
    let would_execute = confidence >= confidence_gate && risk_level != "HIGH" && risk_level != "CRITICAL";

    let reason = build_reason(ctx, &action_type, confidence, confidence_gate, &risk_level);
    let rollback_display = if rollback_action.is_empty() { "none".to_string() } else { rollback_action.clone() };
    ...
    println!();
    if would_execute {
        println!("  {} {}", "Would execute?".bright_white(), "YES".bright_green().bold());
    } else {
        println!("  {} {}", "Would execute?".bright_white(), "NO".bright_red().bold());
    }
    println!("  {} {}", "Outcome:".bright_white(), "NOT EXECUTED (simulation only)".yellow().bold());
    println!();
...
fn classify_action(action: &str) -> String {
    ...
    if a.contains("checkpoint")                          { "auto-checkpoint".into() }
    else if a.contains("restart") || a.contains("start") { "restart-service".into() }
    else if a.contains("cache") || a.contains("clear")   { "clear-cache".into() }
    else if a.contains("health") || a.contains("doctor") { "run-health-check".into() }
    else if a.contains("commit") || a.contains("push")   { "git-commit".into() }
    else if a.contains("delete") || a.contains("remove") { "delete-file".into() }
    else if a.contains("config") || a.contains("modify") { "modify-config".into() }
    else                                                  { "run-health-check".into() }
}
...
fn estimate_confidence(ctx: &AppContext, action_type: &str, _action: &str) -> f64 {
    ...
    let correct: i64 = ctx.runtime.db.query_row(
...
fn build_reason(ctx: &AppContext, action_type: &str, confidence: f64, gate: f64, risk: &str) -> String {
    if risk == "CRITICAL" {
    ...
    if confidence < gate {
        return format!("confidence {:.0}% below gate {:.0}% — more history needed",
            confidence * 100.0, gate * 100.0);
    }
    let sims: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM delegate_simulations WHERE action_type = ?1",
        rusqlite::params![action_type],
        |r: &rusqlite::Row| r.get(0),
    ).unwrap_or(0);
    format!("confidence {:.0}% meets gate, {} prior simulations observed", confidence * 100.0, sims)
}
...
pub fn contracts(ctx: &AppContext) -> CoreResult<()> {
    ...
    let rows = stmt.query_map([], |r: &rusqlite::Row<'_>| Ok((
        r.get::<_,String>(0)?,
        r.get::<_,String>(1)?,
        r.get::<_,f64>(2)?,
        r.get::<_,i64>(3)?,
    ...
    for (action, risk, gate, active) in rows.flatten() {
        let active_str = if active == 1 { "✅".to_string() } else { "○ sim".to_string() };
        println!("  {:<22} {:<10} {:<8} {}",
    ...
    println!();
    println!("  {} All contracts in simulation mode — activation requires 85% accuracy over 14+ days",
        "ℹ".bright_cyan());
    println!();
    Ok(())
}
...
pub fn history(ctx: &AppContext) -> CoreResult<()> {
    ...
    for (desc, atype, conf, risk, would, outcome, ts) in rows.flatten() {
        ...
        let exec = if would == 1 { "YES".bright_green() } else { "NO".bright_red() };
        println!("  {} {} {} conf:{:.0}% {}",
            time.dimmed(), exec, risk_color(&risk),
            conf * 100.0, desc.bright_white());
        println!("       {} → {}", atype.dimmed(), outcome.dimmed());
        count += 1;
    }
    if count == 0 {
        println!("  {} No simulations run yet — try: core delegate simulate restart-faelight-notify",
            "○".dimmed());
    }
    println!();
    Ok(())
...
pub fn accuracy(ctx: &AppContext) -> CoreResult<()> {
    ...
    let total: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM delegate_simulations", [], |r: &rusqlite::Row| r.get(0)
    ).unwrap_or(0);
    let verified: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM delegate_simulations WHERE verified IS NOT NULL", [], |r: &rusqlite::Row| r.get(0)
    ).unwrap_or(0);
    let correct: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM delegate_simulations WHERE verified = 1", [], |r: &rusqlite::Row| r.get(0)
    ).unwrap_or(0);

    let acc = if verified > 0 { correct as f64 / verified as f64 * 100.0 } else { 0.0 };
    let gate_met = acc >= 85.0 && verified >= 14;

    println!("  {}  {}", "Total simulations:".dimmed(), total.to_string().bright_white());
    println!("  {}  {}", "Verified:".dimmed(), verified.to_string().bright_white());
    println!("  {}  {:.1}%", "Accuracy:".dimmed(),
        if verified > 0 { acc } else { 0.0 });
    println!();
    if gate_met {
        println!("  {} Activation gate MET — run: core delegate activate <contract>",
            "✅".green());
    } else {
        println!("  {} Activation gate: need 85% accuracy over 14+ verified simulations",
            "⬜".normal());
        println!("  {} Current: {:.1}% accuracy, {} verified",
            "→".dimmed(), acc, verified);
    }
    println!();
    Ok(())
...
pub fn suspend(ctx: &AppContext) -> CoreResult<()> {
    ensure_tables(ctx);
    ctx.runtime.db.execute(
        "UPDATE delegate_contracts SET active = 0", []
    ).ok();
    println!("  {} All delegation suspended — simulation mode only", "🛑".bright_red());
    Ok(())
}
...
pub fn activate(ctx: &AppContext, contract: &str) -> CoreResult<()> {
    ...
    if verified < 14 || acc < 85.0 {
        println!();
        println!("  {} Activation gate NOT met for: {}", "❌".bright_red(), contract.bright_yellow());
        println!("  {}  Need 14+ verified simulations at 85%+ accuracy", "→".dimmed());
        println!("  {}  Current: {} verified, {:.1}% accuracy", "→".dimmed(), verified, acc);
        println!();
        return Ok(());
    }
    ...
    if matches!(contract, "git-commit" | "delete-file" | "modify-config") {
        println!("  {} '{}' is a hard boundary — activation permanently blocked",
            "🔒".bright_red(), contract);
        return Ok(());
    }
    ...
    println!("  {} Contract activated: {}", "✅".green(), contract.bright_green().bold());
    println!("  {} Monitor closely — suspend anytime with: core delegate suspend", "ℹ".bright_cyan());
    Ok(())
}
...
pub fn counterfactuals(ctx: &AppContext) -> CoreResult<()> {
    ensure_tables(ctx);
    println!();
    println!("{}", "📊 Delegation Counterfactual Log".bright_cyan().bold());
    println!("{}", "━".repeat(60).dimmed());
    println!();
    let mut stmt = ctx.runtime.db.prepare(
    ...
    let rows: Vec<(String, String, i64, f64, Option<i64>, i64)> = stmt.query_map([], |r| {
        Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?, r.get(5)?))
    })?.filter_map(|r| r.ok()).collect();
    if rows.is_empty() {
        println!("  {} No counterfactuals logged yet.", "○".dimmed());
        println!("  {} Run simulations and record your actual actions to build the dataset.", "→".dimmed());
        println!();
        return Ok(());
    }
    for (proposed, human, matched, conf, outcome, ts) in &rows {
        let match_icon = if *matched == 1 { "✓".bright_green() } else { "✗".bright_red() };
        ...
        let date = chrono::DateTime::from_timestamp(*ts, 0)
            .map(|t| t.format("%m-%d %H:%M").to_string())
            .unwrap_or_default();
        println!("  {} {} proposed: {} → human: {} | conf: {:.0}% | outcome: {}",
            match_icon, date.dimmed(),
            proposed.bright_white(), human.bright_yellow(),
            conf * 100.0, outcome_str
        );
    }
    println!();
    // Show accuracy summary
    let acc = DelegateAccuracy::from_db(ctx);
    println!("  {} Action match: {:.0}%  Outcome success: {:.0}%  Calibration error: {:.2}",
        "→".dimmed(),
...
pub fn log_counterfactual(ctx: &AppContext, proposed: &str, human: &str, matched: bool, confidence: f64) -> CoreResult<()> {
    ensure_tables(ctx);
    let now = chrono::Utc::now().timestamp();
    ctx.runtime.db.execute(
    ...
    println!("  {} Counterfactual logged: proposed='{}' human='{}' match={}",
        "✅".green(), proposed, human, if matched { "yes" } else { "no" });
    Ok(())
}
...
pub fn accuracy_report(ctx: &AppContext) -> CoreResult<()> {
    ensure_tables(ctx);
    println!();
    println!("{}", "🎯 Delegation Accuracy — Three Dimensions".bright_cyan().bold());
    println!("{}", "━".repeat(60).dimmed());
    println!();
    let acc = DelegateAccuracy::from_db(ctx);
    let total: i64 = ctx.runtime.db.query_row(
    ...
    println!("  {} {} counterfactuals logged", "📋".normal(), total.to_string().bright_white());
    println!();
    let am_colored = if acc.action_match >= 0.85 { format!("{:.0}%", acc.action_match * 100.0).bright_green() }
        else { format!("{:.0}%", acc.action_match * 100.0).bright_red() };
    let os_colored = if acc.outcome_success >= 0.80 { format!("{:.0}%", acc.outcome_success * 100.0).bright_green() }
        else { format!("{:.0}%", acc.outcome_success * 100.0).bright_red() };
    let ce_colored = if acc.calibration_error <= 0.10 { format!("{:.3}", acc.calibration_error).bright_green() }
        else { format!("{:.3}", acc.calibration_error).bright_red() };
    println!("  {:<30} {} (gate: ≥85%)", "Action match:".bright_white(), am_colored);
    println!("  {:<30} {} (gate: ≥80%)", "Outcome success:".bright_white(), os_colored);
    println!("  {:<30} {} (gate: ≤0.10)", "Calibration error:".bright_white(), ce_colored);
    println!();
    if acc.activation_ready() {
        println!("  {} ALL GATES PASSED — delegation can be activated", "✅".green().bold());
        println!("  {} Run: core delegate activate <contract>", "→".bright_cyan());
    } else {
        println!("  {} Activation gates not yet met:", "⏳".normal());
        if acc.action_match < 0.85 {
            println!("    {} action_match {:.0}% < 85%", "✗".bright_red(), acc.action_match * 100.0);
        }
        if acc.outcome_success < 0.80 {
            println!("    {} outcome_success {:.0}% < 80%", "✗".bright_red(), acc.outcome_success * 100.0);
        }
        if acc.calibration_error > 0.10 {
            println!("    {} calibration_error {:.3} > 0.10", "✗".bright_red(), acc.calibration_error);
        }
        if total < 50 {
            println!("    {} only {} samples — need 50+ for reliable measurement", "💡".bright_cyan(), total);
        }
    }
    println!();
    Ok(())
