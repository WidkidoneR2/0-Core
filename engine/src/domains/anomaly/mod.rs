...
pub fn scan(ctx: &AppContext) -> CoreResult<()> {
    ctx.capabilities
        .require("anomaly", &[Capability::FilesystemReadHome])?;
    ...
    println!(
        "{}",
        "  ╭─ 🔍 Anomaly Detection Scan ─────────────────────────".bright_cyan()
    );
...
pub fn history(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!(
        "{}",
        "  ╭─ 📜 Anomaly History ────────────────────────────────".bright_cyan()
    );

    if rows.is_empty() {
        println!(
            "  │  {} No anomaly scans yet — run: {}",
            "○".dimmed(),
            "core anomaly scan".bright_cyan()
        );
    } else {
        for (payload, ts) in &rows {
            ...
            let count = serde_json::from_str::<serde_json::Value>(payload)
                .ok()
                .and_then(|v| v["detail"]["anomalies"].as_i64())
                .unwrap_or(0);
            let icon = if count == 0 {
                "✅".to_string()
            } else {
...
pub fn alert(ctx: &AppContext) -> CoreResult<()> {
    ...
    if high_count == 0 {
        ...
        println!(
            "{}",
            format!("  {} high-severity anomalies require attention", high_count)
                .bright_red()
                .bold()
        );
        for a in anomalies
...
fn detect_unintented_commits(core_root: &str) -> Vec<Anomaly> {
    ...
    for line in output.lines() {
        ...
        let has_intent = msg.contains("INT-")
            || msg.starts_with("ledger:")
