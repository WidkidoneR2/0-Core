...
fn check_gate(ctx: &AppContext) -> bool {
    ...
    if score < JARVIS_GATE {
        ...
        println!("  {} Jarvis score: {}/100 (gate: {}/100)",
            "→".bright_cyan(), score.to_string().yellow(), JARVIS_GATE);
        println!("  {} Complete INT-159 (context) + INT-160 (memory) to reach gate",
            "→".dimmed());
        println!("  {} Run: core strategy jarvis — to see current score", "→".dimmed());
        println!();
        return false;
    }
    true
...
pub fn mandate_list(ctx: &AppContext) -> CoreResult<()> {
    ...
    let mandates: Vec<(i64, String, String)> = stmt.query_map([], |r| {
    ...
    if mandates.is_empty() {
        ...
        for (id, rule, scope) in &mandates {
            println!("  #{} [{}] {}", id.to_string().dimmed(), scope.bright_cyan(), rule.bright_white());
        }
    }
    ...
    println!();
    if score < JARVIS_GATE {
        println!("  {} Autonomy DORMANT — Jarvis {}/{}", "🔒".normal(), score, JARVIS_GATE);
    } else {
        println!("  {} Autonomy ACTIVE — Jarvis {}/{}", "✅".normal(), score, JARVIS_GATE);
    }
    println!();
    Ok(())
}
...
pub fn mandate_set(ctx: &AppContext, rule: &str) -> CoreResult<()> {
    ...
    println!();
    println!("  {} Mandate recorded: {}", "✅".normal(), rule.bright_white());
    println!("  {} Status: stored, awaiting Jarvis gate ({}/100)", "→".dimmed(), JARVIS_GATE);
    println!("  {} The forest will act on this when score reaches gate", "→".dimmed());
    println!();
    Ok(())
}
...
pub fn mandate_revoke_all(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!();
    println!("  {} All mandates revoked — forest returned to fully manual mode", "🔒".normal());
    println!();
    Ok(())
}
...
pub fn autonomy_pending(ctx: &AppContext) -> CoreResult<()> {
    ensure_tables(ctx)?;
    if !check_gate(ctx) { return Ok(()); }
    println!("  {} No autonomous actions pending", "○".dimmed());
    println!("  {} Mandates define what the forest may suggest", "→".dimmed());
    Ok(())
}
...
pub fn autonomy_log(ctx: &AppContext) -> CoreResult<()> {
    ...
    let entries: Vec<(String, String, String, i64)> = stmt.query_map([], |r| {
    ...
    if entries.is_empty() {
        ...
        for (action, reason, _result, reverted) in &entries {
            let status = if *reverted == 1 { "↩".yellow().to_string() } else { "✅".to_string() };
            println!("  {} {} — {}", status, action.bright_white(), reason.dimmed());
        }
    }
    println!();
    Ok(())
...
pub fn trust_score(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!("  {} Jarvis score:    {}/100", "→".bright_cyan(), jarvis.to_string().bright_white());
    println!("  {} Gate required:   {}/100", "→".dimmed(), JARVIS_GATE);
    println!("  {} Status:          {}", "→".dimmed(),
        if jarvis >= JARVIS_GATE { "ACTIVE".bright_green().to_string() }
        else { format!("DORMANT ({} points to gate)", JARVIS_GATE - jarvis).yellow().to_string() }
    );

    let mandate_count: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM forest_mandates WHERE active=1", [], |r| r.get(0)
    ).unwrap_or(0);
    let action_count: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM autonomy_log", [], |r| r.get(0)
    ).unwrap_or(0);
    println!("  {} Active mandates: {}", "→".dimmed(), mandate_count.to_string().bright_white());
    println!("  {} Actions taken:   {}", "→".dimmed(), action_count.to_string().bright_white());
    println!();
    Ok(())
}
...
pub fn trust_history(ctx: &AppContext) -> CoreResult<()> {
    ensure_tables(ctx)?;
    println!("  {} No trust history yet — autonomy is dormant", "○".dimmed());
    Ok(())
}
...
pub fn trust_expand(ctx: &AppContext) -> CoreResult<()> {
    ensure_tables(ctx)?;
    if !check_gate(ctx) { return Ok(()); }
    println!("  {} Trust expansion available when actions have been verified", "○".dimmed());
    Ok(())
}
