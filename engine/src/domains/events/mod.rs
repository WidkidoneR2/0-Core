...
pub fn replay(ctx: &AppContext, from_seq: i64, to_seq: i64) -> CoreResult<()> {
    ...
    let rows: Vec<(i64, i64, String, String, String, String, Option<i64>, f64)> = stmt
        .query_map(params![from_seq, to_seq], |r| {
            Ok((
                r.get(0)?,
                r.get(1)?,
                r.get(2)?,
...
pub fn status_v2(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!(
        "  {:<25} {}",
        "Total signals:".dimmed(),
        total.to_string().bright_white()
    );
    println!(
        "  {:<25} {}",
        "Max sequence:".dimmed(),
        max_seq.to_string().bright_cyan()
    );
    println!(
        "  {:<25} {} ({:.0}%)",
        "With causality:".dimmed(),
        with_causality.to_string().bright_green(),
        if total > 0 {
            with_causality as f64 / total as f64 * 100.0
        } else {
            0.0
        }
    );
...
pub fn why_focus(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!("  {:8}  {:6}  {:7}  Quality", "Date", "Focus", "Switch");
