...
pub fn update_pattern_weight(
    ...
    ctx.runtime.db.execute(
        ...
        params![
            ...
            metrics.frequency, metrics.recency, metrics.consequence,
            metrics.trend, metrics.volatility, metrics.confidence,
            breakdown.final_weight, class.label(),
            breakdown.base, breakdown.confidence_adjusted,
            breakdown.volatility_adjusted, breakdown.decay_adjusted,
            breakdown.identity_adjusted, now,
            (metrics.frequency * 30.0) as i64, 30i64,
            is_positive as i64
        ],
    )?;
    Ok(breakdown)
...
pub fn list(ctx: &AppContext) -> CoreResult<()> {
    ensure_tables(ctx)?;
    println!();
    println!("{}", "⚖️  Pattern Weights — Ranked by Importance".cyan().bold());
    ...
    let rows: Vec<(String, String, String, f64, String, f64, f64, f64, f64, f64, i64)> =
        stmt.query_map([], |r| {
            Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?,
                r.get(5)?, r.get(6)?, r.get(7)?, r.get(8)?, r.get(9)?, r.get(10)?))
        })?.filter_map(|r| r.ok()).collect();
    if rows.is_empty() {
        println!("  {} No patterns yet — run: core weight compute", "○".dimmed());
        println!("  {} Weights are built from forest events over time", "→".dimmed());
        println!();
        return Ok(());
    }
    println!("  {:<8} {:<14} {:<20} {:<8} {}",
        "Weight".dimmed(), "Class".dimmed(), "Context".dimmed(),
        "Conf".dimmed(), "Description".dimmed());
    ...
    println!();
    println!("  {} Run: core weight explain <id> for full breakdown", "→".bright_cyan());
    println!();
    Ok(())
}
...
pub fn explain(ctx: &AppContext, id: &str) -> CoreResult<()> {
    ensure_tables(ctx)?;
    let row: Option<(String, String, f64, f64, f64, f64, f64, f64, f64, f64, f64, f64, f64, f64)> =
    ...
    match row {
        None => {
            ...
            println!("  {} Run: core weight list to see available patterns", "→".dimmed());
        }
        Some((desc, ctx_name, final_w, freq, rec, cons, trend, vol, conf,
              base, conf_adj, vol_adj, decay_adj, id_adj)) => {
            let class = classify_weight(final_w);
            println!();
            println!("{} Pattern: {}", "⚖️ ".normal(), desc.bright_white().bold());
            println!("{}", "━".repeat(60).dimmed());
            println!("  {} {}  {} {}", "Context:".dimmed(), ctx_name.bright_cyan(),
                "Class:".dimmed(), class.color());
            println!();
            println!("  {}", "Input Metrics:".bright_white().bold());
            println!("  {:<20} {:.3}  (rate = occurrences/window_days)", "frequency:".dimmed(), freq);
            println!("  {:<20} {:.3}  (decay-adjusted)", "recency:".dimmed(), rec);
            println!("  {:<20} {:.3}  (outcome severity)", "consequence:".dimmed(), cons);
            println!("  {:<20} {:.3}  (-1=improving → 1=worsening)", "trend:".dimmed(), trend);
            println!("  {:<20} {:.3}  (0=stable → 1=chaotic)", "volatility:".dimmed(), vol);
            println!("  {:<20} {:.3}  (data reliability)", "confidence:".dimmed(), conf);
            println!();
            println!("  {}", "Weight Breakdown (each stage):".bright_white().bold());
            println!("  {:<28} {:.4}  (weighted dimensions sum)", "base:".dimmed(), base);
            println!("  {:<28} {:.4}  (× confidence)", "confidence_adjusted:".dimmed(), conf_adj);
            println!("  {:<28} {:.4}  (× stability_factor(volatility))", "volatility_adjusted:".dimmed(), vol_adj);
            println!("  {:<28} {:.4}  (× temporal decay)", "decay_adjusted:".dimmed(), decay_adj);
            println!("  {:<28} {:.4}  (× alignment [0.9,1.1])", "identity_adjusted:".dimmed(), id_adj);
            println!();
            let fw: f64 = final_w;
            let weight_colored = if fw >= 0.80 { format!("{:.4}", fw).bright_red().bold() }
                else if fw >= 0.65 { format!("{:.4}", fw).bright_green() }
                else if fw >= 0.45 { format!("{:.4}", fw).bright_yellow() }
                else { format!("{:.4}", fw).dimmed() };
            println!("  {:<28} {} → {}", "FINAL WEIGHT:".bright_white().bold(),
                weight_colored, class.color());
            println!();
...
pub fn compute(ctx: &AppContext) -> CoreResult<()> {
    ...
    let commit_count: i64 = ctx.runtime.db.query_row(
        ...
        params![now - window_secs], |r| r.get(0)
    ).unwrap_or(0);
    let commit_rate = (commit_count as f64 / window_days).min(1.0);
    let commit_recency: f64 = ctx.runtime.db.query_row(
    ...
    let commit_breakdown = update_pattern_weight(
        ctx, "commit-velocity", "Commit velocity — shipping cadence",
        "work_rhythm", &commit_metrics,
        (now - ctx.runtime.db.query_row(
            "SELECT MAX(timestamp) FROM events WHERE domain='git' AND action='commit'",
            [], |r| r.get::<_, Option<i64>>(0)
        ).unwrap_or(None).unwrap_or(now)) as f64 / 3600.0,
        true,
    )?;
    println!("  {} commit-velocity → {:.3} ({})",
        "✓".bright_green(), commit_breakdown.final_weight,
        classify_weight(commit_breakdown.final_weight).label());
    // Pattern 2: health stability
    let health_drops: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM doctor_history WHERE health < 95 AND checked_at > ?1",
        params![now - window_secs], |r| r.get(0)
    ).unwrap_or(0);
    let health_checks: i64 = ctx.runtime.db.query_row(
    ...
    let health_breakdown = update_pattern_weight(
        ctx, "health-drops", "Health drops below 95%",
        "health", &health_metrics, 0.0, false,
    )?;
    println!("  {} health-drops → {:.3} ({})",
        "✓".bright_green(), health_breakdown.final_weight,
    ...
    let intent_breakdown = update_pattern_weight(
        ctx, "intent-load", "Active intent load (focus risk)",
        "work_rhythm", &intent_metrics, 0.0,
        active_intents <= 3,
    )?;
    println!("  {} intent-load → {:.3} ({})",
        "✓".bright_green(), intent_breakdown.final_weight,
        classify_weight(intent_breakdown.final_weight).label());
    // Pattern 4: prediction accuracy
    let total_preds: i64 = ctx.runtime.db.query_row(
        "SELECT COUNT(*) FROM forest_predictions WHERE created_at > ?1",
        params![now - window_secs], |r| r.get(0)
    ).unwrap_or(0);
    let correct_preds: i64 = ctx.runtime.db.query_row(
    ...
    let pred_breakdown = update_pattern_weight(
        ctx, "prediction-accuracy", "Prediction engine accuracy",
        "prediction", &pred_metrics, 0.0,
        accuracy > 0.7,
    )?;
    println!("  {} prediction-accuracy → {:.3} ({})",
        "✓".bright_green(), pred_breakdown.final_weight,
        classify_weight(pred_breakdown.final_weight).label());
    println!();
    println!("  {} Weights computed. Run: core weight list to see rankings", "✅".green());
    println!("  {} Run: core weight explain <id> for full breakdown", "→".bright_cyan());
    println!();
    Ok(())
}
...
pub fn top(ctx: &AppContext) -> CoreResult<()> {
    ...
    let rows: Vec<(String, String, String, f64, String, i64)> = stmt.query_map([], |r| {
        Ok((r.get(0)?, r.get(1)?, r.get(2)?, r.get(3)?, r.get(4)?, r.get(5)?))
    })?.filter_map(|r| r.ok()).collect();
    if rows.is_empty() {
        println!("  {} No high-weight patterns yet", "○".dimmed());
        println!("  {} Run: core weight compute to build pattern weights", "→".dimmed());
    } else {
        for (id, desc, ctx_name, weight, class, positive) in &rows {
            let marker = if *positive == 1 { "✨".to_string() } else { "⚠️ ".to_string() };
            let class_colored = match class.as_str() {
                "CRITICAL" => class.bright_red().bold(),
                _ => class.bright_green(),
            };
            println!("  {} {} [{} | {}] {:.3}",
                marker, desc.bright_white(), class_colored,
                ctx_name.bright_cyan(), weight);
...
pub fn calibrate(ctx: &AppContext, id: &str, outcome: &str) -> CoreResult<()> {
    ensure_tables(ctx)?;
    let pattern: Option<(f64, String)> = ctx.runtime.db.query_row(
    ...
    match pattern {
        ...
        Some((weight, ctx_name)) => {
            let success = matches!(outcome.to_lowercase().as_str(), "success" | "correct" | "yes");
            let now = now_ts();
            ctx.runtime.db.execute(
            ...
            println!("  {} Calibration recorded for '{}': outcome={}, was_correct={}",
                "✅".green(), id, outcome, success);
