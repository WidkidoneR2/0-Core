...
fn read_forecast_trend(ctx: &AppContext) -> f64 {
    ...
    let vals: Vec<f64> = ctx
        .runtime
        .db
        .prepare("SELECT payload FROM events WHERE domain='doctor' ORDER BY timestamp DESC LIMIT 5")
        .ok()
        .map(|mut s| {
...
fn score_goals(ctx: &AppContext) -> Vec<GoalScore> {
    ...
    goals
        ...
        .map(|(id, title, priority)| {
            ...
            let is_stability = title_lower.contains("health")
                || title_lower.contains("stability")
                || title_lower.contains("fix")
                || title_lower.contains("restore");
            if health < 95 {
                if is_stability {
                    score += 0.3;
                    factors.push((
            ...
            if security_findings > 5 {
                let is_security = title_lower.contains("security")
                ...
                if is_security {
                    ...
                    factors.push((
                        ...
                        0.2,
                        format!(
                            "{} findings open — security goals elevated",
                            security_findings
                        ),
                    ));
                } else {
...
pub fn prioritize(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!(
        "  {} {}",
        "⚖  Dynamic Prioritization".bright_cyan().bold(),
    ...
    println!("{}", "─".repeat(56).dimmed());
    println!(
        "  {} run {} for factor breakdown",
        "→".dimmed(),
        "core prioritize explain".bright_cyan()
    );
    println!("{}", "━".repeat(56).dimmed());
    println!();
...
pub fn explain(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!(
        "  {} {}",
        "⚖  Prioritization Explained".bright_cyan().bold(),
        format!("(health: {}%)", health).dimmed()
    );
