...
pub fn run(ctx: &AppContext, since: Option<&str>, intent: Option<&str>) -> CoreResult<()> {
    ctx.capabilities
        .require("narrative", &[Capability::FilesystemReadHome])?;
...
fn full_narrative(ctx: &AppContext, since: Option<&str>) -> CoreResult<()> {
    ...
    println!(
        "  │  {} commits and {} custom Rust tools.",
        commit_count.bright_white(),
    ...
    let decisions: Vec<(String, String)> = {
        let mut stmt = ctx
            .runtime
            .db
            .prepare("SELECT description, outcome FROM decisions ORDER BY timestamp ASC")
            .ok();
    ...
    if decisions.is_empty() {
        println!(
            "  │  No decisions recorded yet — use {} to begin",
    ...
    println!(
        "  │  {} intents have been completed.",
        complete_count.to_string().bright_green()
    );
    println!(
        "  │  {} intents guide the path forward.",
        future_count.to_string().bright_yellow()
    );
    ...
    println!(
        "  │  {}",
        "Chapter V — The Core Timeline".bright_white().bold()
    );
    println!("  │");
    ...
    println!(
        "  │  {}",
        "Chapter VI — Present Moment".bright_white().bold()
    );
    println!("  │");
    ...
    let ts = std::time::SystemTime::now()
        ...
        .unwrap_or(0);
    let payload = format!(
        r#"{{"actor":"core","result":"ok","detail":{{"command":"narrative","since":"{}"}}}}"#,
...
fn intent_narrative(ctx: &AppContext, intent_id: &str) -> CoreResult<()> {
    ...
    if let Some(file) = intent_file {
        ...
        println!(
            "  │  {} commits reference this intent:",
            commit_count.to_string().bright_white()
        );
