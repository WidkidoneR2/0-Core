...
fn status(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!(
        "  {:<24} {}/100 (gate: {})",
        "v14 Gate:".dimmed(),
        gate_pct.to_string().bright_white(),
        V14_JARVIS_GATE
    );
    ...
    println!(
        ...
        "Status:".dimmed(),
        if activated {
            "ACTIVE — partnership engaged".bright_green()
        } else {
            "DORMANT — building toward activation".bright_yellow()
        }
    );

    // Count activity
    let proposals: i64 = ctx
        .runtime
        .db
        .query_row("SELECT COUNT(*) FROM partner_proposals", [], |r| r.get(0))
        .unwrap_or(0);
    let disagreements: i64 = ctx
        .runtime
        .db
        .query_row("SELECT COUNT(*) FROM partner_disagreements", [], |r| {
            r.get(0)
        })
        .unwrap_or(0);
    ...
    println!(
        "  {:<24} {}",
        "Proposals made:".dimmed(),
        proposals.to_string().bright_white()
    );
    println!(
        "  {:<24} {}",
        "Disagreements recorded:".dimmed(),
        disagreements.to_string().bright_white()
    );
    println!();
    ...
    let phases = [
        (
            "Phase 1 — Collaborative Intent",
            "propose / discuss / disagree",
        ),
        ("Phase 2 — Shared Decision Making", "consult"),
        (
            "Phase 3 — Longitudinal Memory",
            "reflect / pattern / growth",
        ),
...
fn propose(ctx: &AppContext) -> CoreResult<()> {
    ...
    if new_proposals.is_empty() {
        println!(
            "  {} No new proposals this week — recent proposals still pending",
            "○".dimmed()
        );
    } else {
...
fn discuss(ctx: &AppContext, intent_id: &str) -> CoreResult<()> {
    ...
    let opinion = match &intent_content {
        None => format!(
            "Intent {} not found in ledger — verify the ID is correct.",
            intent_id
        ),
        Some(content) => {
            let title = content
            ...
            format!(
                ...
                if gates_done == gates_total {
                    ...
                    "Gates remaining — continue building."
                }
            )
        }
    };
...
fn consult(ctx: &AppContext, question: &str) -> CoreResult<()> {
    ...
    let response = if q.contains("start") || q.contains("begin") || q.contains("cistart") {
        ...
        if in_progress >= 5 {
            ...
            format!(
                "{} intents in-progress — within range. Proceeding is reasonable.",
                in_progress
            )
        }
...
fn reflect(ctx: &AppContext) -> CoreResult<()> {
    ...
    let success_rate = if events > 0 {
        (succeeded * 100) / events
    } else {
        0
    };
...
fn pattern(ctx: &AppContext) -> CoreResult<()> {
    ...
    if timestamps.is_empty() {
        println!(
            "  {} Not enough data yet — patterns emerge over time",
            "○".dimmed()
        );
    } else {
...
fn growth(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!(
        "  {:<28} v9 → v10 → v11 → v12 → v13 → v14",
        "Intelligence arc:".dimmed()
    );
    println!(
        "  {:<28} {}/100",
        "Jarvis Score:".dimmed(),
...
fn roadmap(ctx: &AppContext) -> CoreResult<()> {
    ...
    let v14_note = format!(
        "requires {}/100 Jarvis -- {} more points",
        V14_JARVIS_GATE,
        (V14_JARVIS_GATE - score).max(0)
    );
    let steps: Vec<(&str, &str, &str)> = vec![
        (
            "NOW",
            "INT-187 Delegation Engine",
            "trust contracts accumulating -- keep running",
        ),
        (
            "NOW",
        ...
        (
            "MAY 3",
            "Jarvis 100/100",
            "shell intelligence full +10 -- complete daily driver gate",
        ),
...
fn roadmap_why(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!(
        "  {} INT-179 clock — shell intelligence score unlocks at May 3",
        "2.".bright_white()
    );
    println!(
        "  {} Tool retirement — cleaner registry improves reality-check accuracy",
        "3.".bright_white()
    );
    println!(
    ...
    println!(
        "  {} Journal — system autobiography required for longitudinal memory (v14 Phase 3)",
        "5.".bright_white()
    );
    println!();
    Ok(())
...
fn roadmap_diff(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!();
    println!(
        "  {} No other significant divergences detected",
        "✅".normal()
    );
    println!();
    Ok(())
