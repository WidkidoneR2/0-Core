            ...
            for entry in entries.flatten() {
                ...
                let depends_on = content.lines()
                    ...
                    .map(|l| l.trim_start_matches("depends_on:")
                        ...
                        .collect())
                    .unwrap_or_default();
                nodes.push(IntentNode { id, title, status, spawned_by, spawns, depends_on });
            }
        }
    }
    nodes
...
pub fn show(ctx: &AppContext, id: &str) -> CoreResult<()> {
    ...
    let target = match map.get(id) {
        ...
        None => {
            println!();
            println!("  {} Intent {} not found", "❌".bright_red(), id.bright_white());
            println!("  {} Run: core genealogy tree — to see all intents", "·".dimmed());
            println!();
            return Ok(());
        }
    };
    ...
    println!("  {} {}", "▶".bright_cyan(), "Ancestors:".bright_white().bold());
    ...
    if ancestors.is_empty() {
        ...
        for (i, anc_id) in ancestors.iter().enumerate() {
            if let Some(anc) = map.get(anc_id) {
                let indent = "  ".repeat(i);
                println!("    {}{} INT-{}: {}", indent,
                    status_icon(&anc.status), anc_id.bright_white(), anc.title.dimmed());
            }
        }
        // Show target
    ...
    println!("  {} {}", "▶".bright_cyan(), "Descendants:".bright_white().bold());
    fn print_descendants(
        node_id: &str,
        map: &HashMap<String, &IntentNode>,
        depth: usize,
    ) {
        if let Some(node) = map.get(node_id) {
            for child_id in &node.spawns {
                if let Some(child) = map.get(child_id) {
                    ...
                    println!("    {}{} INT-{}: {}",
                        indent, status_icon(&child.status),
                        child_id.bright_white(), child.title.dimmed());
    ...
    if !target.depends_on.is_empty() {
        println!("  {} {}", "▶".bright_cyan(), "Depends on:".bright_white().bold());
        for dep_id in &target.depends_on {
            if let Some(dep) = map.get(dep_id) {
                println!("    {} INT-{}: {}", status_icon(&dep.status),
                    dep_id.bright_white(), dep.title.dimmed());
            }
        }
        println!();
    }
...
pub fn tree(ctx: &AppContext) -> CoreResult<()> {
    ...
    let mut roots: Vec<&IntentNode> = nodes.iter()
    ...
    fn print_tree(
        ...
        if let Some(node) = map.get(node_id) {
            let indent = if depth == 0 {
                ...
                if is_last { format!("{}  └── ", base) } else { format!("{}  ├── ", base) }
            };
            println!("  {}{} INT-{}: {}",
                indent, status_icon(&node.status),
                node_id.bright_white(), node.title.dimmed());
    ...
    if roots.is_empty() {
        ...
        println!("  {} Add spawned_by/spawns fields to intent frontmatter", "→".bright_green());
    } else {
        for root in &roots {
    ...
    println!("  {} Legend: ✅ complete  🔄 in-progress  ⬜ planned", "·".dimmed());
    println!();
    Ok(())
}
...
pub fn roots(ctx: &AppContext) -> CoreResult<()> {
    ...
    let roots: Vec<&IntentNode> = nodes.iter()
        .filter(|n| n.spawned_by.is_none())
        .collect();
    ...
    for node in &roots_sorted {
        let spawns_str = if node.spawns.is_empty() {
            ...
            format!("spawns: {}", node.spawns.join(", ")).bright_green().to_string()
        };
        println!("  {} INT-{}: {}  {}",
