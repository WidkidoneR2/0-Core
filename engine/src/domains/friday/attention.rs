...
pub fn compute_novelty(db: &Connection, event_type: &str, event_detail: &str) -> f64 {
    let count: i64 = db.query_row(
        ...
        rusqlite::params![event_type, format!("%{}%", &event_detail[..event_detail.len().min(20)])],
        |r| r.get(0),
    ).unwrap_or(0);
...
pub fn compute_risk(event_type: &str, event_detail: &str) -> f64 {
    ...
    match event_type {
        "health_drop" => {
            ...
            if detail_lower.contains("below 80") || detail_lower.contains("critical") { 1.0 }
            else if detail_lower.contains("below 90") { 0.9 }
            else { 0.7 }
        }
        "schema_change" => 0.9,
        ...
        "contradiction" => 0.8,
        "deploy" => {
            if detail_lower.contains("core") || detail_lower.contains("faelight-shell") { 0.5 }
            else { 0.2 }
        }
        "pattern_match" => 0.4,
        "idle_timeout" => 0.1,
...
pub fn compute_strategic_relevance(db: &Connection, event_detail: &str) -> f64 {
    ...
    match active_intent {
        ...
        Some(intent) => {
            ...
            if intent_lower.split_whitespace().any(|w| w.len() > 4 && detail_lower.contains(w)) {
                0.9
...
pub fn compute_temporal_pressure(db: &Connection) -> f64 {
    ...
    let session_events: i64 = db.query_row(
        "SELECT COUNT(*) FROM events WHERE timestamp > strftime('%s','now') - 3600",
        [], |r| r.get(0),
    ).unwrap_or(0);

    if session_events > 50 { 0.6 }       // Active deep session
    else if session_events > 20 { 0.4 }  // Normal session
    else if session_events > 5 { 0.3 }   // Light session
    else { 0.1 }                          // Idle / just started
}
...
pub fn record_attention(
    ...
    let _ = db.execute(
        "INSERT INTO friday_attention
         ...
         VALUES (?,?,?,?,?,?,?,?,?,?,?,?,?)",
        rusqlite::params![
            now, &event.event_type, &event.event_detail,
            event.novelty, event.risk, event.strategic_relevance,
            event.uncertainty, event.temporal_pressure,
            score, ATTENTION_THRESHOLD,
            if spoke { 1 } else { 0 },
...
pub fn attention_stats(db: &Connection) -> (i64, i64, f64) {
    let total: i64 = db.query_row(
        "SELECT COUNT(*) FROM friday_attention", [], |r| r.get(0)
    ).unwrap_or(0);
    let spoke: i64 = db.query_row(
        "SELECT COUNT(*) FROM friday_attention WHERE spoke = 1", [], |r| r.get(0)
    ).unwrap_or(0);
    let avg_score: f64 = db.query_row(
        "SELECT AVG(attention_score) FROM friday_attention", [], |r| r.get(0)
    ).unwrap_or(0.0);
...
pub fn show_debug(db: &Connection) {
    println!();
    println!("  {} {}", "🌲 Friday Attention Debug".bright_green().bold(), "Core v24".dimmed());
    println!("{}", "  ━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━".dimmed());
    ...
    println!("  {} Spoke: {} ({:.0}% silence rate)", "→".bright_cyan(), spoke, silence_rate);
    println!("  {} Avg attention score: {:.3}", "→".bright_cyan(), avg);
    println!("  {} Threshold: {:.1} (speak) / {:.1} (interrupt)",
        "→".bright_cyan(), ATTENTION_THRESHOLD, INTERRUPT_THRESHOLD);
    println!();
    // Recent events
    ...
    }).unwrap().flatten().for_each(|(etype, score, spoke, detail)| {
        let spoke_str = if spoke == 1 { "spoke".bright_yellow() } else { "silent".dimmed() };
