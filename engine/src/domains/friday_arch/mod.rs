...
pub fn detect_patterns(ctx: &AppContext) -> CoreResult<Vec<String>> {
    ...
    if active_intents > 2 {
        let desc = format!("CONTRADICTION: {} active intents -- values declare focus>speed", active_intents);
    ...
    if commits_today > 15 {
        let desc = format!("HIGH VELOCITY: {} commits today -- monitor health trajectory", commits_today);
    ...
    let deploy_success: i64 = db.query_row(
    ...
    if deploy_count > 3 {
        ...
        if success_rate < 0.8 {
            let desc = format!("SIGNAL: Deploy success rate {:.0}% -- investigate before continuing",
                success_rate * 100.0);
            patterns_found.push(desc);
        }
    }
    ...
    let recent_health: Vec<i64> = {
        let mut s = db.prepare(
            "SELECT health FROM synthesis_snapshots ORDER BY timestamp DESC LIMIT 5"
        ).unwrap();
    ...
    if recent_health.len() >= 3 {
        ...
        if trend < -5 {
            patterns_found.push(format!("TREND: Health declining {} points across last {} snapshots",
                trend.abs(), recent_health.len()));
        }
    }
    Ok(patterns_found)
}
...
pub fn show_models(ctx: &AppContext) -> CoreResult<()> {
    ...
    let rows: Vec<(i64, String, String, f64, i64, i64)> = {
        ...
        let x: Vec<(i64, String, String, f64, i64, i64)> = s.query_map([], |r| Ok((
            r.get(0)?, r.get::<_,String>(1)?, r.get::<_,String>(2)?,
    ...
    println!();
    println!("  {} Friday Models -- Meta-Interpretation Engine", "🌲".normal());
    println!("  {}", "─".repeat(60).dimmed());
    for (id, desc, domain, accuracy, predictions, correct) in &rows {
        let short_desc = desc.chars().take(55).collect::<String>();
        let acc_colored = if *accuracy >= 0.8 { format!("{:.0}%", accuracy*100.0).bright_green().to_string() }
            else if *accuracy >= 0.6 { format!("{:.0}%", accuracy*100.0).bright_yellow().to_string() }
            else { format!("{:.0}%", accuracy*100.0).bright_red().to_string() };
        println!("  {} [{}] {} ({}) {}/{} {}",
            "→".bright_green(), id.to_string().dimmed(),
            short_desc.white(), domain.dimmed(),
            correct, predictions, acc_colored);
    }
    println!();
    Ok(())
}
...
pub fn run(ctx: &AppContext) -> CoreResult<()> {
    ...
    if !patterns.is_empty() {
        ...
        for p in &patterns {
            let colored = if p.starts_with("CONTRADICTION") { p.bright_red().to_string() }
                else if p.starts_with("SIGNAL") { p.bright_yellow().to_string() }
                else if p.starts_with("TREND") { p.bright_yellow().to_string() }
                else { p.bright_white().to_string() };
            println!("    → {}", colored);
        }
    } else {
        println!("  {} No cross-layer patterns detected -- forest is coherent", "✅".green());
    }
    ...
    if !contradictions.is_empty() {
        ...
        for (a, b, desc) in &contradictions {
            println!("    {} {} ↔ {}: {}", "→".bright_red(), a.bright_cyan(), b.bright_cyan(),
    ...
    let brief = if !patterns.is_empty() {
        format!("Friday sees {} cross-layer signal(s). {}{}",
            patterns.len(),
            if !contradictions.is_empty() { format!("{} contradiction(s) active. ", contradictions.len()) } else { String::new() },
            patterns.first().map(|p| p.chars().take(80).collect::<String>()).unwrap_or_default())
    } else {
    ...
    let _ = ctx.runtime.db.execute(
        ...
        params![now, brief, if contradictions.is_empty() { "" } else { "yes" }],
    );
    println!();
...
pub fn show_proposals(ctx: &AppContext) -> CoreResult<()> {
    ...
    println!();
    println!("  {} Friday Proposals -- Pending Human Review", "🌲".normal());
    println!("  {}", "─".repeat(55).dimmed());
    if rows.is_empty() {
        println!("  {} No pending proposals.", "→".dimmed());
    } else {
        for (id, desc, action, conf) in &rows {
            println!("  [{}] {} ({:.0}%)", id, desc.chars().take(50).collect::<String>(), conf*100.0);
